<?php
// ============================================================
// NATIONAL COORDINATOR - CREATE BROADCAST WITH EMAIL
// ============================================================
...
// Get parameters
$target_state = isset($_GET['state']) ? intval($_GET['state']) : 0;
$target_states = isset($_GET['states']) ? explode(',', $_GET['states']) : [];
...
$roles = [];
try {
    $stmt = $db->prepare("SELECT id, name, level FROM roles WHERE is_active = 1 ORDER BY name ASC");
    $stmt->execute();
    $roles = $stmt->fetchAll();
} catch (Exception $e) {
    $roles = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        try {
            ...
            // ============================================================
            // SEND EMAILS IF SEND_NOW IS CHECKED AND EMAIL IS IN SEND_VIA
            // ============================================================
            if ($send_now && in_array('email', $send_via)) {
                ...
                // Update broadcast with sent counts
                $stmt = $db->prepare("
                    UPDATE broadcasts 
                    SET sent_at = NOW(), 
                        status = 'sent',
                ...
                $message = "Broadcast created and sent to " . $email_sent_count . " recipients!";
                ...
                <div>
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <select name="target_audience" class="form-control" id="targetAudience" 
                            ...
                            <option value="state" <?php echo ($_POST['target_audience'] ?? '') === 'state' ? 'selected' : ''; ?>>Specific States</option>
                            <option value="role_specific" <?php echo ($_POST['target_audience'] ?? '') === 'role_specific' ? 'selected' : ''; ?>>Specific Roles</option>
                        </select>
                    </div>
                    
                    <!-- State Selection -->
                    <div id="stateSelection" style="margin-bottom:16px;display:<?php echo ($_POST['target_audience'] ?? '') === 'state' ? 'block' : 'none'; ?>;">
                        <label style="display:block;font-weight:600;font-size:0.85rem;color:var(--gray-700);margin-bottom:4px;">
                            Select States <span style="color:#EF4444;">*</span>
                        </label>
                        <select name="target_ids[]" multiple class="form-control" 
                            ...
                            <?php foreach ($states as $state): ?>
                                <option value="<?php echo $state['id']; ?>" 
                                    <?php echo (isset($_POST['target_ids']) && in_array($state['id'], (array)$_POST['target_ids'])) || 
                                            $target_state == $state['id'] || 
                                            in_array($state['id'], $target_states) ? 'selected' : ''; ?>>
                        ...
                        <select name="target_ids[]" multiple class="form-control" 
                            ...
                            <?php foreach ($roles as $role): ?>
                                <option value="<?php echo $role['id']; ?>" 
                                    ...
                                    <?php echo htmlspecialchars($role['name']); ?> (<?php echo htmlspecialchars($role['level']); ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <div style="font-size:0.65rem;color:var(--gray-400);margin-top:4px;">
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <?php 
                        // Initialize send_via with default if not set
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (targetAudience) {
        targetAudience.addEventListener('change', function() {
            ...
            stateSelection.style.display = value === 'state' ? 'block' : 'none';
