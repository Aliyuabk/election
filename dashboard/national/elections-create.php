<?php
// ============================================================
// NATIONAL COORDINATOR - CREATE ELECTION
// ============================================================
        ...
        try {
            ...
            $stmt = $db->prepare("
                INSERT INTO elections (
                    ...
                    states_json, lgas_json, wards_json, pus_json,
    ...
    <div class="main-content-inner">
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-top:8px;">
                <div>
                    ...
                    <p style="color:var(--gray-500);margin:2px 0 0;">
                        Set up a new election with locations and configuration
                    </p>
                </div>
        ...
        <form method="POST" action="" style="background:white;border-radius:var(--radius);padding:24px;border:1px solid var(--gray-200);">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
                ...
                <div>
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <select name="type" class="form-control" required
                            ...
                            <?php foreach ($election_types as $key => $label): ?>
                                <option value="<?php echo $key; ?>" <?php echo ($_POST['type'] ?? '') == $key ? 'selected' : ''; ?>>
                                    <?php echo $label; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <input type="text" name="cycle" class="form-control"
                               value="<?php echo htmlspecialchars($_POST['cycle'] ?? ''); ?>"
                ...
                <div>
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <div style="margin-bottom:8px;">
                            <input type="date" name="election_date" class="form-control" required
                                   value="<?php echo htmlspecialchars($_POST['election_date'] ?? ''); ?>"
                                   style="width:100%;padding:10px 14px;border:1px solid var(--gray-200);border-radius:10px;font-family:'Inter',sans-serif;font-size:0.85rem;transition:var(--transition);">
                        </div>
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <!-- States -->
                        <div style="margin-bottom:8px;">
                            <select name="states[]" multiple class="form-control" id="statesSelect"
                                ...
                                <?php foreach ($states as $state): ?>
                                    <option value="<?php echo $state['id']; ?>"
                                        <?php echo (isset($_POST['states']) && in_array($state['id'], (array)$_POST['states'])) ? 'selected' : ''; ?>>
                            ...
                            <select name="lgas[]" multiple class="form-control"
                                ...
                                <?php foreach ($lgas as $lga): ?>
                                    <option value="<?php echo $lga['id']; ?>"
                                        <?php echo (isset($_POST['lgas']) && in_array($lga['id'], (array)$_POST['lgas'])) ? 'selected' : ''; ?>>
                            ...
                            <select name="wards[]" multiple class="form-control"
                                ...
                                <?php foreach ($wards as $ward): ?>
                                    <option value="<?php echo $ward['id']; ?>"
                                        <?php echo (isset($_POST['wards']) && in_array($ward['id'], (array)$_POST['wards'])) ? 'selected' : ''; ?>>
                            ...
                            <select name="pus[]" multiple class="form-control"
                                ...
                                <?php foreach ($polling_units as $pu): ?>
                                    <option value="<?php echo $pu['id']; ?>"
                                        <?php echo (isset($_POST['pus']) && in_array($pu['id'], (array)$_POST['pus'])) ? 'selected' : ''; ?>>
            ...
            <div style="display:flex;gap:12px;margin-top:24px;padding-top:20px;border-top:1px solid var(--gray-200);flex-wrap:wrap;">
                ...
                <a href="elections.php" class="btn-secondary" style="padding:10px 32px;background:transparent;color:var(--gray-500);border:1px solid var(--gray-200);border-radius:10px;text-decoration:none;font-family:'Inter',sans-serif;font-weight:500;font-size:0.85rem;cursor:pointer;transition:var(--transition);display:flex;align-items:center;gap:8px;">
        ...
        <div style="background:#F0FDF4;border-radius:var(--radius);padding:16px 20px;margin-top:20px;border:1px solid #A7F3D0;">
            <h4 style="font-size:0.85rem;font-weight:600;color:#065F46;margin:0 0 8px;">
                <i class="fas fa-info-circle"></i> Election Setup Tips
            </h4>
            <ul style="font-size:0.8rem;color:#065F46;margin:0;padding-left:20px;">
                <li>Select at least one location (State, LGA, Ward, or Polling Unit)</li>
                <li>Use "Draft" status to save and edit later</li>
                <li>Set to "Upcoming" when ready to schedule</li>
                <li>Set to "Active" when the election is live</li>
                <li>You can add templates to reuse election configurations</li>
            </ul>
        </div>
    </div>
</main>
