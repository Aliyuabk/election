<?php
// ============================================================
// NATIONAL COORDINATOR - POLLING UNIT CHECK-INS
// ============================================================
    ...
    <div class="main-content-inner">
        ...
        <div style="background:white;border-radius:var(--radius);padding:16px 20px;margin-bottom:20px;border:1px solid var(--gray-200);">
            <form method="GET" action="" style="display:flex;flex-wrap:wrap;gap:12px;align-items:center;">
                ...
                <div style="min-width:180px;">
                    <select name="agent" class="form-select" style="width:100%;padding:8px 14px;border:1px solid var(--gray-200);border-radius:10px;font-family:'Inter',sans-serif;font-size:0.8rem;background:white;">
                ...
                <?php if ($agent_id > 0): ?>
                    <a href="?id=<?php echo $pu_id; ?>" class="btn-reset" style="padding:8px 16px;background:var(--gray-100);color:var(--gray-600);border:none;border-radius:10px;font-family:'Inter',sans-serif;font-weight:500;font-size:0.8rem;cursor:pointer;text-decoration:none;transition:var(--transition);">
        ...
        <div style="background:white;border-radius:var(--radius);overflow:hidden;border:1px solid var(--gray-200);">
            <div style="padding:12px 20px;border-bottom:1px solid var(--gray-200);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                ...
                <span style="font-size:0.75rem;color:var(--gray-500);"><?php echo count($checkins); ?> of <?php echo number_format($total_checkins); ?></span>
            </div>
                    ...
                    <table style="width:100%;border-collapse:collapse;font-size:0.8rem;">
                        <thead style="background:var(--gray-50);border-bottom:2px solid var(--gray-200);">
                            <tr>
                                ...
                                <th style="padding:10px 14px;text-align:center;font-weight:600;color:var(--gray-600);">Check-in Type</th>
                                <th style="padding:10px 14px;text-align:center;font-weight:600;color:var(--gray-600);">GPS Location</th>
                                <th style="padding:10px 14px;text-align:center;font-weight:600;color:var(--gray-600);">Distance</th>
                                <th style="padding:10px 14px;text-align:center;font-weight:600;color:var(--gray-600);">Device</th>
                                <th style="padding:10px 14px;text-align:center;font-weight:600;color:var(--gray-600);">Battery</th>
                                <th style="padding:10px 14px;text-align:center;font-weight:600;color:var(--gray-600);">Time</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr style="border-bottom:1px solid var(--gray-100);transition:var(--transition);hover:background:var(--gray-50);">
                                    <td style="padding:10px 14px;">
                                        <div style="display:flex;align-items:center;gap:10px;">
                                            <div style="width:32px;height:32px;border-radius:50%;background:var(--secondary);color:white;display:flex;align-items:center;justify-content:center;font-weight:600;font-size:0.7rem;flex-shrink:0;">
                                                <?php echo strtoupper(substr($checkin['agent_name'] ?? 'U', 0, 1)); ?>
                                            </div>
                                            <div>
                                                <div style="font-weight:500;font-size:0.85rem;"><?php echo htmlspecialchars($checkin['agent_name'] ?? 'Unknown'); ?></div>
                                                <div style="font-size:0.65rem;color:var(--gray-400);"><?php echo htmlspecialchars($checkin['agent_email'] ?? 'N/A'); ?></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td style="padding:10px 14px;text-align:center;">
                                        <span style="display:inline-block;padding:2px 12px;border-radius:12px;font-size:0.65rem;font-weight:600;background:<?php echo $type_color; ?>20;color:<?php echo $type_color; ?>;">
                                    ...
                                    <td style="padding:10px 14px;text-align:center;font-size:0.75rem;">
                                        <?php if (!empty($checkin['gps_lat']) && !empty($checkin['gps_lng'])): ?>
                                            ...
                                            <div style="font-size:0.6rem;color:var(--gray-400);"><?php echo number_format($checkin['gps_lng'], 6); ?></div>
                                            <?php if (!empty($checkin['gps_accuracy'])): ?>
                                                <div style="font-size:0.55rem;color:var(--gray-400);">±<?php echo $checkin['gps_accuracy']; ?>m</div>
                                            <?php endif; ?>
                                        <?php else: ?>
                                    ...
                                    <td style="padding:10px 14px;text-align:center;font-size:0.7rem;color:var(--gray-500);">
                                        <?php if (!empty($checkin['device_id'])): ?>
                                            <div><?php echo htmlspecialchars(substr($checkin['device_id'], 0, 12)) . '...'; ?></div>
                                    ...
                                    <td style="padding:10px 14px;text-align:center;">
                                        <?php if (!empty($checkin['device_battery'])): ?>
                                            <div style="display:flex;align-items:center;gap:4px;justify-content:center;">
                                                <div style="width:30px;height:12px;background:var(--gray-200);border-radius:4px;overflow:hidden;border:1px solid var(--gray-300);">
                                                    <div style="width:<?php echo $checkin['device_battery']; ?>%;height:100%;background:<?php echo $checkin['device_battery'] > 50 ? '#10B981' : ($checkin['device_battery'] > 20 ? '#F59E0B' : '#EF4444'); ?>;border-radius:4px;"></div>
                                                </div>
                                                <span style="font-size:0.7rem;font-weight:600;"><?php echo $checkin['device_battery']; ?>%</span>
                                            </div>
                                        <?php else: ?>
                                            <span style="color:var(--gray-400);">—</span>
                                        <?php endif; ?>
                                    ...
                                    <td style="padding:10px 14px;text-align:center;font-size:0.7rem;color:var(--gray-500);">
                                        ...
                                        <?php if ($checkin['is_offline_sync']): ?>
                                            <span style="font-size:0.55rem;background:#FEF3C7;color:#92400E;padding:1px 6px;border-radius:8px;">
            ...
            <?php else: ?>
                <div style="padding:60px 20px;text-align:center;color:var(--gray-500);">
                    <i class="fas fa-sign-in-alt" style="font-size:3rem;display:block;margin-bottom:12px;color:var(--gray-300);"></i>
                    <h3 style="font-size:1.1rem;font-weight:600;color:var(--gray-600);margin:0 0 8px;">No Check-ins Found</h3>
                    <p style="font-size:0.85rem;color:var(--gray-400);margin:0;">No check-in records found for this polling unit.</p>
                </div>
            <?php endif; ?>
        </div>
        ...
        <?php if ($total_pages > 1): ?>
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;padding:12px 0;">
                <div style="font-size:0.8rem;color:var(--gray-500);">
                    Showing <?php echo (($page - 1) * $limit) + 1; ?> - <?php echo min($page * $limit, $total_checkins); ?> of <?php echo number_format($total_checkins); ?> check-ins
                </div>
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                    ...
                    if ($start_page > 1) {
                        echo '<a href="?id=' . $pu_id . '&page=1&agent=' . $agent_id . '" class="btn-page" style="padding:6px 12px;border:1px solid var(--gray-200);border-radius:8px;text-decoration:none;color:var(--gray-600);font-size:0.8rem;transition:var(--transition);">1</a>';
                    ...
                    if ($end_page < $total_pages) {
                        ...
                        echo '<a href="?id=' . $pu_id . '&page=' . $total_pages . '&agent=' . $agent_id . '" class="btn-page" style="padding:6px 12px;border:1px solid var(--gray-200);border-radius:8px;text-decoration:none;color:var(--gray-600);font-size:0.8rem;transition:var(--transition);">' . $total_pages . '</a>';
                    }
                    ?>
