<?php
// ============================================================
// NATIONAL COORDINATOR - EC8E RESULTS (NATIONAL)
// ============================================================
...
try {
    ...
    // Count total
    $count_stmt = $db->prepare("SELECT COUNT(*) FROM results_ec8e r WHERE $where_sql");
    ...
    error_log("EC8E Results Error: " . $e->getMessage());
}
...
$page_title = 'EC8E Results';
$page_subtitle = 'National Collation Form';
?>
    ...
    <div class="main-content-inner">
        ...
        <div class="welcome-section">
            <div class="breadcrumb">
                ...
                <i class="fas fa-chevron-right" style="font-size:0.6rem;color:var(--gray-400);"></i>
                <span style="font-weight:600;color:var(--gray-800);">EC8E</span>
            </div>
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-top:8px;">
                <div>
                    ...
                    <p style="color:var(--gray-500);margin:2px 0 0;">
                        ...
                        National Collation Form • <?php echo number_format($total_results); ?> records
                    </p>
                </div>
                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                    ...
                    <a href="reports.php?type=ec8e" class="btn-primary" style="padding:8px 16px;background:var(--primary);color:white;border:none;border-radius:10px;text-decoration:none;font-weight:600;font-size:0.8rem;display:inline-flex;align-items:center;gap:6px;">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-label">Total EC8E</div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-number">
                    ...
                    echo number_format($declared);
                    ?>
                </div>
                <div class="stat-label">Declared</div>
                <div class="stat-change"><i class="fas fa-flag"></i> Final</div>
            </div>
            <div class="stat-card">
        ...
        $national_valid_votes = 0;
        $national_rejected_votes = 0;
        foreach ($results as $r) {
            $national_total_votes += $r['total_votes'] ?? 0;
            $national_valid_votes += $r['valid_votes'] ?? 0;
            $national_rejected_votes += $r['rejected_votes'] ?? 0;
        }
        ?>
        <?php if ($total_results > 0): ?>
        <div style="background:white;border-radius:var(--radius);padding:16px 20px;border:1px solid var(--gray-200);margin-bottom:16px;">
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;">
                <div style="text-align:center;padding:8px;background:var(--gray-50);border-radius:8px;">
                    <div style="font-size:1.5rem;font-weight:700;color:var(--primary);"><?php echo number_format($national_total_votes); ?></div>
                    <div style="font-size:0.65rem;color:var(--gray-500);">Total National Votes</div>
                </div>
                <div style="text-align:center;padding:8px;background:var(--gray-50);border-radius:8px;">
                    <div style="font-size:1.5rem;font-weight:700;color:#10B981;"><?php echo number_format($national_valid_votes); ?></div>
                    <div style="font-size:0.65rem;color:var(--gray-500);">Valid Votes</div>
                </div>
                <div style="text-align:center;padding:8px;background:var(--gray-50);border-radius:8px;">
                    <div style="font-size:1.5rem;font-weight:700;color:#EF4444;"><?php echo number_format($national_rejected_votes); ?></div>
                    <div style="font-size:0.65rem;color:var(--gray-500);">Rejected Votes</div>
                </div>
                <div style="text-align:center;padding:8px;background:var(--gray-50);border-radius:8px;">
                    <div style="font-size:1.5rem;font-weight:700;color:#8B5CF6;"><?php echo number_format($total_results); ?></div>
                    <div style="font-size:0.65rem;color:var(--gray-500);">Total Submissions</div>
                </div>
            </div>
        </div>
        <?php endif; ?>
            ...
            <form method="GET" action="" style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;">
                <div style="flex:1;min-width:150px;">
                    <div class="search-box" style="width:100%;">
                        ...
                        <input type="text" name="search" placeholder="Search coordinator..." value="<?php echo htmlspecialchars($search); ?>" />
                    </div>
                </div>
                ...
                <?php if (!empty($search) || !empty($status_filter)): ?>
                    <a href="results-ec8e.php" class="btn-reset" style="padding:6px 12px;background:var(--gray-100);color:var(--gray-600);border:none;border-radius:8px;font-family:'Inter',sans-serif;font-weight:500;font-size:0.75rem;cursor:pointer;text-decoration:none;transition:var(--transition);">
        ...
        <div style="background:white;border-radius:var(--radius);overflow:hidden;border:1px solid var(--gray-200);">
                    ...
                    <table style="width:100%;border-collapse:collapse;font-size:0.75rem;">
                        <thead style="background:var(--gray-50);border-bottom:2px solid var(--gray-200);">
                            <tr>
                                ...
                                <th style="padding:8px 12px;text-align:center;font-weight:600;color:var(--gray-600);">Declaration</th>
                            ...
                            ?>
                                <tr style="border-bottom:1px solid var(--gray-100);transition:var(--transition);hover:background:var(--gray-50);">
                                    <td style="padding:8px 12px;">
                                        <div style="font-weight:500;font-size:0.8rem;"><?php echo htmlspecialchars($result['coordinator_name'] ?? 'Unknown'); ?></div>
                                        <div style="font-size:0.6rem;color:var(--gray-400);">
                                            <i class="fas fa-phone"></i> <?php echo htmlspecialchars($result['coordinator_phone'] ?? 'N/A'); ?>
                                        </div>
                                    </td>
                                    <td style="padding:8px 12px;text-align:center;">
                                        ...
                                        <div style="font-size:0.55rem;color:var(--gray-400);">
                                            ...
                                            <?php if ($result['rejected_votes'] > 0): ?>
                                                • Rej: <?php echo number_format($result['rejected_votes']); ?>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                    <td style="padding:8px 12px;text-align:center;">
                                    ...
                                    <td style="padding:8px 12px;text-align:center;font-size:0.7rem;">
                                        <?php if ($result['declaration_time']): ?>
                                            <div style="font-weight:500;color:#8B5CF6;">Declared</div>
                                            <div style="font-size:0.55rem;color:var(--gray-400);">
                                                <?php echo date('M j, Y g:i A', strtotime($result['declaration_time'])); ?>
                                            </div>
                                        <?php else: ?>
                                    ...
                                    <td style="padding:8px 12px;text-align:center;">
                                        <div style="display:flex;gap:3px;justify-content:center;flex-wrap:wrap;">
                                            <a href="result-view.php?type=ec8e&id=<?php echo $result['id']; ?>" class="btn-sm" style="padding:2px 8px;border-radius:4px;background:var(--primary);color:white;text-decoration:none;font-size:0.6rem;" title="View">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <?php if ($result['status'] === 'pending'): ?>
                                                <a href="ec8e-verify.php?id=<?php echo $result['id']; ?>" class="btn-sm" style="padding:2px 8px;border-radius:4px;background:#10B981;color:white;text-decoration:none;font-size:0.6rem;" title="Verify">
                                                    <i class="fas fa-check"></i>
                                                </a>
                                                <a href="ec8e-declare.php?id=<?php echo $result['id']; ?>" class="btn-sm" style="padding:2px 8px;border-radius:4px;background:#8B5CF6;color:white;text-decoration:none;font-size:0.6rem;" title="Declare">
                                                    <i class="fas fa-bullhorn"></i>
                                                </a>
                                            <?php endif; ?>
                                            <?php if ($result['status'] === 'flagged'): ?>
                                                <a href="ec8e-verify.php?id=<?php echo $result['id']; ?>" class="btn-sm" style="padding:2px 8px;border-radius:4px;background:#10B981;color:white;text-decoration:none;font-size:0.6rem;" title="Verify">
                                                    <i class="fas fa-check"></i>
                                                </a>
                                            <?php endif; ?>
            ...
            <?php else: ?>
                <div style="padding:40px 20px;text-align:center;color:var(--gray-500);">
                    ...
                    <p style="font-size:0.85rem;">No EC8E results found.</p>
                </div>
            <?php endif; ?>
        </div>
        ...
        <?php if ($total_pages > 1): ?>
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;padding:10px 0;">
                ...
                <div style="display:flex;gap:4px;flex-wrap:wrap;">
                    ...
                    if ($start_page > 1) {
                        echo '<a href="?page=1&search=' . urlencode($search) . '&status=' . urlencode($status_filter) . '" class="btn-page" style="padding:4px 10px;border:1px solid var(--gray-200);border-radius:6px;text-decoration:none;color:var(--gray-600);font-size:0.7rem;transition:var(--transition);">1</a>';
                    ...
                    if ($end_page < $total_pages) {
                        ...
                        echo '<a href="?page=' . $total_pages . '&search=' . urlencode($search) . '&status=' . urlencode($status_filter) . '" class="btn-page" style="padding:4px 10px;border:1px solid var(--gray-200);border-radius:6px;text-decoration:none;color:var(--gray-600);font-size:0.7rem;transition:var(--transition);">' . $total_pages . '</a>';
                    }
                    ?>
