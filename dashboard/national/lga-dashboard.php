<?php
// ============================================================
// NATIONAL COORDINATOR - LGA DASHBOARD VIEW
// ============================================================
...
// If no LGA ID or invalid, redirect to monitor-states
if ($lga_id <= 0) {
...
// ============================================================
// FIRST - Check if LGA exists and get state info
// ============================================================
$lga_data = null;
$state_name = '';
$state_id = 0;
...
try {
    ...
    error_log("LGA Dashboard Check Error: " . $e->getMessage());
    header('Location: monitor-states.php?error=database_error');
    exit();
}

// ============================================================
// FETCH FULL LGA DATA WITH STATISTICS
// ============================================================
...
try {
    ...
    $lga_data['total_wards'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - Wards count error: " . $e->getMessage());
    $lga_data['total_wards'] = 0;
}
...
try {
    ...
    $stmt->execute([$lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['total_pus'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - PUs count error: " . $e->getMessage());
    $lga_data['total_pus'] = 0;
}

// Get Ward Coordinators count
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM users u JOIN roles r ON u.role_id = r.id WHERE u.tenant_id = ? AND r.level = 'ward' AND u.jurisdiction_id IN (SELECT id FROM wards WHERE lga_id = ?) AND u.status = 'active' AND (u.deleted_at IS NULL OR u.deleted_at = '0000-00-00 00:00:00')");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['ward_coordinators'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - Ward coordinators error: " . $e->getMessage());
    $lga_data['ward_coordinators'] = 0;
}

// Get PU Agents count
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM users u JOIN roles r ON u.role_id = r.id WHERE u.tenant_id = ? AND r.level = 'pu_agent' AND u.jurisdiction_id IN (SELECT id FROM polling_units WHERE ward_id IN (SELECT id FROM wards WHERE lga_id = ?)) AND u.status = 'active' AND (u.deleted_at IS NULL OR u.deleted_at = '0000-00-00 00:00:00')");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['pu_agents'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - PU agents error: " . $e->getMessage());
    $lga_data['pu_agents'] = 0;
}

// Get LGA Coordinator
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM users u JOIN roles r ON u.role_id = r.id WHERE u.tenant_id = ? AND r.level = 'lga' AND u.jurisdiction_id = ? AND u.status = 'active' AND (u.deleted_at IS NULL OR u.deleted_at = '0000-00-00 00:00:00')");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['lga_coordinator'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - LGA coordinator error: " . $e->getMessage());
    $lga_data['lga_coordinator'] = 0;
}

// Get Elections count
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM elections WHERE tenant_id = ? AND deleted_at IS NULL AND lgas_json IS NOT NULL AND lgas_json != ''");
    $stmt->execute([$tenant_id]);
    $total_elections = $stmt->fetch(PDO::FETCH_ASSOC)['count'] ?? 0;
    
    if ($total_elections > 0) {
        $stmt = $db->prepare("SELECT COUNT(*) as count FROM elections WHERE tenant_id = ? AND deleted_at IS NULL AND lgas_json IS NOT NULL AND lgas_json != '' AND JSON_CONTAINS(lgas_json, JSON_QUOTE(?))");
        $stmt->execute([$tenant_id, $lga_id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        $lga_data['election_count'] = $result['count'] ?? 0;
    } else {
        $lga_data['election_count'] = 0;
    }
} catch (Exception $e) {
    error_log("LGA Dashboard - Elections count error: " . $e->getMessage());
    $lga_data['election_count'] = 0;
}

// Get Active Elections
try {
    if (($lga_data['election_count'] ?? 0) > 0) {
        $stmt = $db->prepare("SELECT COUNT(*) as count FROM elections WHERE tenant_id = ? AND deleted_at IS NULL AND status = 'active' AND lgas_json IS NOT NULL AND lgas_json != '' AND JSON_CONTAINS(lgas_json, JSON_QUOTE(?))");
        $stmt->execute([$tenant_id, $lga_id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        $lga_data['active_elections'] = $result['count'] ?? 0;
    } else {
        $lga_data['active_elections'] = 0;
    }
} catch (Exception $e) {
    error_log("LGA Dashboard - Active elections error: " . $e->getMessage());
    $lga_data['active_elections'] = 0;
}

// Get Total Results
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM results_ec8a r WHERE r.tenant_id = ? AND r.lga_id = ?");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['total_results'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - Total results error: " . $e->getMessage());
    $lga_data['total_results'] = 0;
}

// Get Verified Results
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM results_ec8a r WHERE r.tenant_id = ? AND r.lga_id = ? AND r.status = 'verified'");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['verified_results'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - Verified results error: " . $e->getMessage());
    $lga_data['verified_results'] = 0;
}

// Get Pending Results
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM results_ec8a r WHERE r.tenant_id = ? AND r.lga_id = ? AND r.status = 'pending'");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['pending_results'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - Pending results error: " . $e->getMessage());
    $lga_data['pending_results'] = 0;
}

// Get Flagged Results
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM results_ec8a r WHERE r.tenant_id = ? AND r.lga_id = ? AND r.status = 'flagged'");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['flagged_results'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - Flagged results error: " . $e->getMessage());
    $lga_data['flagged_results'] = 0;
}

// Get EC8B Results
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM results_ec8b WHERE tenant_id = ? AND lga_id = ?");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['ec8b_count'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - EC8B count error: " . $e->getMessage());
    $lga_data['ec8b_count'] = 0;
}

// Get Total Incidents
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM incidents WHERE tenant_id = ? AND lga_id = ?");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['total_incidents'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - Total incidents error: " . $e->getMessage());
    $lga_data['total_incidents'] = 0;
}

// Get Open Incidents
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM incidents WHERE tenant_id = ? AND lga_id = ? AND status IN ('reported', 'investigating')");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['open_incidents'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - Open incidents error: " . $e->getMessage());
    $lga_data['open_incidents'] = 0;
}

// Get Resolved Incidents
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM incidents WHERE tenant_id = ? AND lga_id = ? AND status = 'resolved'");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['resolved_incidents'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - Resolved incidents error: " . $e->getMessage());
    $lga_data['resolved_incidents'] = 0;
}

// Get Critical Incidents
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM incidents WHERE tenant_id = ? AND lga_id = ? AND severity = 'critical'");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['critical_incidents'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - Critical incidents error: " . $e->getMessage());
    $lga_data['critical_incidents'] = 0;
}

// Get Active Assignments
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM agent_assignments WHERE tenant_id = ? AND lga_id = ? AND status = 'active'");
    $stmt->execute([$tenant_id, $lga_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $lga_data['active_assignments'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("LGA Dashboard - Active assignments error: " . $e->getMessage());
    $lga_data['active_assignments'] = 0;
}
...
try {
    $stmt = $db->prepare("
        SELECT 
            ...
            w.code,
            w.registered_voters,
            (SELECT COUNT(*) FROM polling_units pu WHERE pu.ward_id = w.id AND pu.is_active = 1) as pu_count,
            ...
            (SELECT COUNT(*) FROM results_ec8a r WHERE r.tenant_id = ? AND r.ward_id = w.id) as total_results,
            (SELECT COUNT(*) FROM results_ec8a r WHERE r.tenant_id = ? AND r.ward_id = w.id AND r.status = 'verified') as verified_results,
            (SELECT COUNT(*) FROM results_ec8a r WHERE r.tenant_id = ? AND r.ward_id = w.id AND r.status = 'pending') as pending_results,
            (SELECT COUNT(*) FROM incidents i WHERE i.tenant_id = ? AND i.ward_id = w.id) as incidents
        FROM wards w
        WHERE w.lga_id = ? AND w.is_active = 1
        ORDER BY w.name ASC
    ");
    $stmt->execute([$tenant_id, $tenant_id, $tenant_id, $tenant_id, $tenant_id, $tenant_id, $lga_id]);
    $wards = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    error_log("LGA Dashboard - Wards fetch error: " . $e->getMessage());
    $wards = [];
}
...
try {
    $stmt = $db->prepare("
        ...
        WHERE a.tenant_id = ? 
        AND (a.entity_type = 'lga' AND a.entity_id = ?)
        OR (a.entity_type IN ('ward', 'pu') AND a.entity_id IN (
        ...
        LIMIT 15
    ");
    $stmt->execute([$tenant_id, $lga_id, $lga_id, $lga_id]);
    $recent_activities = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    error_log("LGA Dashboard - Recent activities error: " . $e->getMessage());
    $recent_activities = [];
}
...
// CALCULATE PROGRESS
// ============================================================
$total_pus = $lga_data['total_pus'] ?? 0;
$verified_results = $lga_data['verified_results'] ?? 0;
    ...
    <div class="main-content-inner">
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-top:8px;">
                <div>
                    <h2 style="font-size:1.5rem;font-weight:700;margin:0;">
                        <?php echo htmlspecialchars($lga_data['name'] ?? 'LGA'); ?>
                        <span style="font-size:0.7rem;background:<?php echo ($lga_data['is_active'] ?? 0) ? 'var(--primary)' : '#6B7280'; ?>;color:white;padding:2px 12px;border-radius:20px;font-weight:500;margin-left:8px;">
                            <?php echo ($lga_data['is_active'] ?? 0) ? 'Active' : 'Inactive'; ?>
                        </span>
                    </h2>
                    <p style="color:var(--gray-500);margin:2px 0 0;">
                        <i class="fas fa-flag"></i> <?php echo htmlspecialchars($state_name); ?> State
                        <?php if (($lga_data['registered_voters'] ?? 0) > 0): ?>
                            • <i class="fas fa-users"></i> <?php echo number_format($lga_data['registered_voters'] ?? 0); ?> registered voters
                        <?php endif; ?>
                ...
                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                    ...
                    <a href="state-dashboard.php?id=<?php echo $state_id; ?>" class="btn-secondary" style="padding:8px 20px;background:var(--gray-100);color:var(--gray-700);border:1px solid var(--gray-200);border-radius:10px;text-decoration:none;font-weight:500;font-size:0.8rem;display:inline-flex;align-items:center;gap:6px;">
                        <i class="fas fa-tachometer-alt"></i> State Dashboard
                    </a>
                </div>
            </div>
        </div>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:12px;">
                <div>
                    ...
                    <p style="font-size:0.75rem;color:var(--gray-500);margin:2px 0 0;">
                        <?php echo number_format($lga_data['verified_results'] ?? 0); ?> of <?php echo number_format($lga_data['total_pus'] ?? 0); ?> polling units verified
                    </p>
                </div>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-number"><?php echo number_format($lga_data['total_wards'] ?? 0); ?></div>
                <div class="stat-label">Wards</div>
                <div class="stat-change"><i class="fas fa-user-tie"></i> <?php echo $lga_data['ward_coordinators'] ?? 0; ?> coordinators</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-icon green"><i class="fas fa-flag-checkered"></i></div>
                <div class="stat-number"><?php echo number_format($lga_data['total_pus'] ?? 0); ?></div>
                <div class="stat-label">Polling Units</div>
                <div class="stat-change"><i class="fas fa-users"></i> <?php echo $lga_data['pu_agents'] ?? 0; ?> agents</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-icon purple"><i class="fas fa-user-tie"></i></div>
                <div class="stat-number"><?php echo number_format($lga_data['lga_coordinator'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-number"><?php echo number_format($lga_data['election_count'] ?? 0); ?></div>
                <div class="stat-label">Elections</div>
                <div class="stat-change up"><i class="fas fa-play"></i> <?php echo $lga_data['active_elections'] ?? 0; ?> active</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-icon yellow"><i class="fas fa-check-double"></i></div>
                <div class="stat-number"><?php echo number_format($lga_data['verified_results'] ?? 0); ?></div>
                <div class="stat-label">Verified Results</div>
                <div class="stat-change down"><i class="fas fa-clock"></i> <?php echo $lga_data['pending_results'] ?? 0; ?> pending</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-icon teal"><i class="fas fa-file-alt"></i></div>
                <div class="stat-number"><?php echo number_format($lga_data['ec8b_count'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-number"><?php echo number_format($lga_data['total_incidents'] ?? 0); ?></div>
                <div class="stat-label">Incidents</div>
                <div class="stat-change down"><i class="fas fa-clock"></i> <?php echo $lga_data['open_incidents'] ?? 0; ?> open</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-icon pink"><i class="fas fa-clipboard-list"></i></div>
                <div class="stat-number"><?php echo number_format($lga_data['active_assignments'] ?? 0); ?></div>
        ...
        <!-- Incident Summary -->
        <?php if (($lga_data['total_incidents'] ?? 0) > 0): ?>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                ...
                <div style="display:flex;gap:16px;flex-wrap:wrap;">
                    <div style="display:flex;align-items:center;gap:6px;font-size:0.8rem;">
                        <span class="badge badge-danger"><?php echo $lga_data['critical_incidents'] ?? 0; ?></span>
                        <span style="color:var(--gray-500);">Critical</span>
                    </div>
                    <div style="display:flex;align-items:center;gap:6px;font-size:0.8rem;">
                        <span class="badge badge-warning"><?php echo $lga_data['open_incidents'] ?? 0; ?></span>
                        <span style="color:var(--gray-500);">Open</span>
                    </div>
                    <div style="display:flex;align-items:center;gap:6px;font-size:0.8rem;">
                        <span class="badge badge-success"><?php echo $lga_data['resolved_incidents'] ?? 0; ?></span>
                            ...
                            ?>
                                <tr style="border-bottom:1px solid var(--gray-100);transition:var(--transition);hover:background:var(--gray-50);">
                                    <td style="padding:10px 14px;">
                                        ...
                                        <?php endif; ?>
                                        <?php if (($ward['registered_voters'] ?? 0) > 0): ?>
                                            <div style="font-size:0.65rem;color:var(--gray-400);">Voters: <?php echo number_format($ward['registered_voters']); ?></div>
                                        <?php endif; ?>
                                    </td>
        ...
        <div style="background:white;border-radius:var(--radius);padding:16px 20px;border:1px solid var(--gray-200);margin-bottom:20px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
                ...
                <a href="activity-logs.php?lga=<?php echo $lga_id; ?>" style="font-size:0.7rem;color:var(--primary);text-decoration:none;">View All →</a>
            </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;">
            <a href="wards.php?lga=<?php echo $lga_id; ?>" class="quick-action-btn" style="display:flex;align-items:center;gap:10px;padding:12px 16px;background:white;border-radius:var(--radius);border:1px solid var(--gray-200);text-decoration:none;color:var(--gray-700);font-weight:500;transition:var(--transition);">
            ...
            <a href="incidents.php?lga=<?php echo $lga_id; ?>" class="quick-action-btn" style="display:flex;align-items:center;gap:10px;padding:12px 16px;background:white;border-radius:var(--radius);border:1px solid var(--gray-200);text-decoration:none;color:var(--gray-700);font-weight:500;transition:var(--transition);">
                <i class="fas fa-exclamation-triangle" style="color:var(--danger);"></i>
