<?php
// ============================================================
// NATIONAL COORDINATOR - MANAGE POLLING UNITS
// ============================================================
...
if ($ward_id > 0) {
    try {
        $stmt = $db->prepare("
            SELECT w.name as ward_name, w.lga_id, l.name as lga_name, l.state_id, s.name as state_name
            FROM wards w
        ...
        if ($result) {
            ...
            $state_id_found = $result['state_id'];
    ...
    try {
        $stmt = $db->prepare("
            SELECT l.name as lga_name, l.state_id, s.name as state_name
            FROM lgas l
            JOIN states s ON l.state_id = s.id
            WHERE l.id = ?
        ");
...
if ($ward_id > 0) {
    ...
    $params[] = $lga_id_found;
} elseif ($state_id_found > 0) {
    $where_clauses[] = 'l.state_id = ?';
    $params[] = $state_id_found;
}
    ...
    $stmt = $db->prepare("
        SELECT 
            ...
            (SELECT COUNT(*) FROM users u 
             ...
             AND u.jurisdiction_id = pu.id AND u.status = 'active') as agent_count,
            (SELECT COUNT(*) FROM results_ec8a r2 
             WHERE r2.pu_id = pu.id AND r2.tenant_id = ?) as result_count,
            (SELECT COUNT(*) FROM results_ec8a r2 
             WHERE r2.pu_id = pu.id AND r2.tenant_id = ? AND r2.status = 'verified') as verified_count
        FROM polling_units pu
        JOIN wards w ON pu.ward_id = w.id
        JOIN lgas l ON w.lga_id = l.id
        JOIN states s ON l.state_id = s.id
    ...
    <div class="main-content-inner">
        ...
        <div class="welcome-section">
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-top:8px;">
                ...
                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                    <a href="polling-unit-create.php?<?php echo $ward_id > 0 ? 'ward=' . $ward_id : ($lga_id_found > 0 ? 'lga=' . $lga_id_found : ($state_id_found > 0 ? 'state=' . $state_id_found : '')); ?>" class="btn-primary" style="padding:8px 20px;background:var(--primary);color:white;border:none;border-radius:10px;text-decoration:none;font-weight:600;font-size:0.8rem;display:inline-flex;align-items:center;gap:6px;">
        ...
        <div style="background:white;border-radius:var(--radius);padding:16px 20px;margin-bottom:20px;border:1px solid var(--gray-200);">
            <form method="GET" action="" style="display:flex;flex-wrap:wrap;gap:12px;align-items:center;">
                <?php if ($ward_id > 0): ?>
                    <input type="hidden" name="ward" value="<?php echo $ward_id; ?>">
                <?php elseif ($lga_id_found > 0): ?>
                    <input type="hidden" name="lga" value="<?php echo $lga_id_found; ?>">
                <?php elseif ($state_id_found > 0): ?>
                    <input type="hidden" name="state" value="<?php echo $state_id_found; ?>">
                <?php endif; ?>
                
                <div style="flex:1;min-width:200px;">
                    <div class="search-box" style="width:100%;">
                        <i class="fas fa-search"></i>
                        <input type="text" name="search" placeholder="Search by name, code, or description..." value="<?php echo htmlspecialchars($search); ?>" />
                    </div>
                </div>
                
                <?php if ($lga_id_found > 0): ?>
                <div style="min-width:150px;">
                    <select name="ward" class="form-select" style="width:100%;padding:8px 14px;border:1px solid var(--gray-200);border-radius:10px;font-family:'Inter',sans-serif;font-size:0.8rem;background:white;">
                ...
                <?php if (!empty($search) || ($ward_id > 0 && $lga_id_found > 0)): ?>
                    <a href="?<?php echo $ward_id > 0 ? 'ward=' . $ward_id : ($lga_id_found > 0 ? 'lga=' . $lga_id_found : ($state_id_found > 0 ? 'state=' . $state_id_found : '')); ?>" class="btn-reset" style="padding:8px 16px;background:var(--gray-100);color:var(--gray-600);border:none;border-radius:10px;font-family:'Inter',sans-serif;font-weight:500;font-size:0.8rem;cursor:pointer;text-decoration:none;transition:var(--transition);">
        ...
        <div style="background:white;border-radius:var(--radius);overflow:hidden;border:1px solid var(--gray-200);">
            <div style="padding:12px 20px;border-bottom:1px solid var(--gray-200);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                ...
                <span style="font-size:0.75rem;color:var(--gray-500);"><?php echo count($polling_units); ?> of <?php echo number_format($total_polling_units); ?></span>
            </div>
                            ...
                            ?>
                                <tr style="border-bottom:1px solid var(--gray-100);transition:var(--transition);hover:background:var(--gray-50);">
                                    <td style="padding:10px 14px;">
                                        <div style="font-weight:500;font-size:0.85rem;"><?php echo htmlspecialchars($pu['name']); ?></div>
                                        <?php if (!empty($pu['description'])): ?>
                                            <div style="font-size:0.65rem;color:var(--gray-400);"><?php echo htmlspecialchars(substr($pu['description'], 0, 50)) . (strlen($pu['description']) > 50 ? '...' : ''); ?></div>
                                        <?php endif; ?>
                                        <?php if ($pu['is_rural']): ?>
                                            <span style="font-size:0.55rem;background:#FEF3C7;color:#92400E;padding:1px 6px;border-radius:8px;">Rural</span>
                                        <?php endif; ?>
                                    </td>
                                    <td style="padding:10px 14px;text-align:center;font-size:0.8rem;font-weight:500;color:var(--gray-600);">
                                        <?php echo htmlspecialchars($pu['code'] ?? 'N/A'); ?>
                                    </td>
                                    <td style="padding:10px 14px;text-align:center;font-size:0.7rem;">
                                        <div><?php echo htmlspecialchars($pu['ward_name']); ?></div>
                                        <div style="font-size:0.6rem;color:var(--gray-400);"><?php echo htmlspecialchars($pu['lga_name']); ?></div>
                                    </td>
                                    ...
                                    <td style="padding:10px 14px;text-align:center;">
                                        <div><?php echo number_format($pu['registered_voters'] ?? 0); ?></div>
                                        <?php if (($pu['accredited_voters'] ?? 0) > 0): ?>
                                            <div style="font-size:0.6rem;color:var(--gray-400);">
                                                Accred: <?php echo number_format($pu['accredited_voters']); ?>
                                            </div>
                                        <?php endif; ?>
                                    </td>
                                    <td style="padding:10px 14px;text-align:center;">
                                        <?php if (!empty($pu['network_quality'])): ?>
                                            <span style="display:inline-block;padding:2px 8px;border-radius:8px;font-size:0.6rem;font-weight:600;background:<?php echo $network_color; ?>20;color:<?php echo $network_color; ?>;">
                                                <?php echo $network_label; ?>
                                            </span>
                                        <?php else: ?>
                                            <span style="color:var(--gray-400);font-size:0.6rem;">N/A</span>
                                        <?php endif; ?>
                                    </td>
                                    <td style="padding:10px 14px;text-align:center;">
                                        <div style="display:flex;gap:4px;justify-content:center;flex-wrap:wrap;">
                                            <a href="pu-dashboard.php?id=<?php echo $pu['id']; ?>" class="btn-sm" style="padding:3px 10px;border-radius:6px;background:var(--primary);color:white;text-decoration:none;font-size:0.65rem;" title="Dashboard">
                                                <i class="fas fa-tachometer-alt"></i>
                                            </a>
                                            <a href="polling-unit-edit.php?id=<?php echo $pu['id']; ?>" class="btn-sm" style="padding:3px 10px;border-radius:6px;background:var(--gray-200);color:var(--gray-700);text-decoration:none;font-size:0.65rem;" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="pu-agents.php?pu=<?php echo $pu['id']; ?>" class="btn-sm" style="padding:3px 10px;border-radius:6px;background:var(--secondary);color:white;text-decoration:none;font-size:0.65rem;" title="Agents">
                                                <i class="fas fa-user-tie"></i>
                                            </a>
                                            <a href="pu-results.php?id=<?php echo $pu['id']; ?>" class="btn-sm" style="padding:3px 10px;border-radius:6px;background:var(--warning);color:white;text-decoration:none;font-size:0.65rem;" title="Results">
            ...
            <?php else: ?>
                <div style="padding:60px 20px;text-align:center;color:var(--gray-500);">
                    <i class="fas fa-flag-checkered" style="font-size:3rem;display:block;margin-bottom:12px;color:var(--gray-300);"></i>
                    <h3 style="font-size:1.1rem;font-weight:600;color:var(--gray-600);margin:0 0 8px;">No Polling Units Found</h3>
                    <p style="font-size:0.85rem;color:var(--gray-400);margin:0;">No polling units match your current filters.</p>
                    <?php if ($location_name): ?>
                        <a href="polling-unit-create.php?<?php echo $ward_id > 0 ? 'ward=' . $ward_id : ($lga_id_found > 0 ? 'lga=' . $lga_id_found : ($state_id_found > 0 ? 'state=' . $state_id_found : '')); ?>" class="btn-primary" style="padding:10px 28px;background:var(--primary);color:white;border:none;border-radius:10px;text-decoration:none;font-weight:600;font-size:0.85rem;display:inline-flex;align-items:center;gap:8px;margin-top:16px;">
        ...
        <?php if ($total_pages > 1): ?>
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;padding:12px 0;">
                <div style="font-size:0.8rem;color:var(--gray-500);">
                    Showing <?php echo (($page - 1) * $limit) + 1; ?> - <?php echo min($page * $limit, $total_polling_units); ?> of <?php echo number_format($total_polling_units); ?> polling units
                </div>
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                    <?php if ($page > 1): ?>
                        <a href="?page=<?php echo $page - 1; ?>&ward=<?php echo $ward_id; ?>&lga=<?php echo $lga_id_found; ?>&state=<?php echo $state_id_found; ?>&search=<?php echo urlencode($search); ?>" 
                    ...
                    if ($start_page > 1) {
                        echo '<a href="?page=1&ward=' . $ward_id . '&lga=' . $lga_id_found . '&state=' . $state_id_found . '&search=' . urlencode($search) . '" class="btn-page" style="padding:6px 12px;border:1px solid var(--gray-200);border-radius:8px;text-decoration:none;color:var(--gray-600);font-size:0.8rem;transition:var(--transition);">1</a>';
                        if ($start_page > 2) echo '<span style="padding:6px 8px;color:var(--gray-400);">...</span>';
                    }
                    for ($i = $start_page; $i <= $end_page; $i++):
                    ?>
                        <a href="?page=<?php echo $i; ?>&ward=<?php echo $ward_id; ?>&lga=<?php echo $lga_id_found; ?>&state=<?php echo $state_id_found; ?>&search=<?php echo urlencode($search); ?>" 
                    ...
                    if ($end_page < $total_pages) {
                        ...
                        echo '<a href="?page=' . $total_pages . '&ward=' . $ward_id . '&lga=' . $lga_id_found . '&state=' . $state_id_found . '&search=' . urlencode($search) . '" class="btn-page" style="padding:6px 12px;border:1px solid var(--gray-200);border-radius:8px;text-decoration:none;color:var(--gray-600);font-size:0.8rem;transition:var(--transition);">' . $total_pages . '</a>';
                    }
                    ?>
