<?php
// ============================================================
// NATIONAL COORDINATOR - SUSPEND COORDINATOR
// ============================================================
...
try {
    ...
    error_log("Coordinator Suspend Error: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$confirm) {
        $error = 'Please confirm that you want to suspend this coordinator';
    } elseif (empty($suspend_reason)) {
        ...
        try {
            ...
            $log_stmt = $db->prepare("
                ...
                VALUES (?, ?, 'user_suspended', ?, 'user', ?, NOW())
            ");
            $log_stmt->execute([
                $user_id,
                $tenant_id,
                "Suspended coordinator: " . $coordinator['full_name'] . " - Reason: $suspend_reason",
                $coordinator_id
            ]);
            ...
            $security_stmt->execute([
                $tenant_id,
                $coordinator_id,
                "Coordinator suspended by " . $user_name . " - Reason: $suspend_reason",
                getClientIP()
            ]);
            
            $success = true;
            $message = "Coordinator " . $coordinator['full_name'] . " has been suspended successfully!";
            
            // Redirect after success
            header("Location: " . $back_url . "&suspended=1");
            exit();
            
        } catch (Exception $e) {
            $error = 'Failed to suspend coordinator: ' . $e->getMessage();
            error_log("Suspend Coordinator Error: " . $e->getMessage());
        }
    }
}
    ...
    <div class="main-content-inner">
        ...
        <div class="welcome-section">
            <div class="breadcrumb">
                ...
                <i class="fas fa-chevron-right" style="font-size:0.6rem;color:var(--gray-400);"></i>
                <span style="font-weight:600;color:var(--gray-800);">Suspend Coordinator</span>
            </div>
            
            <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-top:8px;">
                <div>
                    <h2 style="font-size:1.5rem;font-weight:700;margin:0;color:var(--danger);">
        ...
        <!-- Warning Box -->
        <div style="background:#FEF2F2;border-radius:var(--radius);padding:16px 20px;margin-bottom:20px;border:1px solid #FECACA;display:flex;align-items:flex-start;gap:12px;">
            <i class="fas fa-exclamation-triangle" style="color:#DC2626;font-size:1.2rem;margin-top:2px;"></i>
            <div>
                <h4 style="font-size:0.9rem;font-weight:600;color:#991B1B;margin:0 0 4px;">Warning: Suspension Action</h4>
                <p style="font-size:0.8rem;color:#991B1B;margin:0;">
                    Suspending this coordinator will:
                </p>
                <ul style="font-size:0.8rem;color:#991B1B;margin:4px 0 0;padding-left:20px;">
                    <li>Immediately revoke all access to the platform</li>
                    <li>Prevent them from submitting results or reporting incidents</li>
                    <li>Remove them from all active assignments</li>
        ...
        <div style="background:white;border-radius:var(--radius);padding:16px 20px;border:1px solid var(--gray-200);margin-bottom:20px;">
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;">
                ...
                <div>
                    ...
                    <div>
                        <span style="display:inline-block;padding:2px 12px;border-radius:12px;font-size:0.7rem;font-weight:600;background:<?php echo ($coordinator['status'] ?? '') === 'active' ? '#D1FAE5' : '#FEE2E2'; ?>;color:<?php echo ($coordinator['status'] ?? '') === 'active' ? '#065F46' : '#991B1B'; ?>;">
                ...
                <div>
                    <label style="display:block;font-size:0.65rem;color:var(--gray-400);text-transform:uppercase;letter-spacing:0.03em;">Last Login</label>
                    <div style="font-weight:500;"><?php echo date('M j, Y g:i A', strtotime($coordinator['last_login_at'])); ?></div>
                </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <form method="POST" action="" style="background:white;border-radius:var(--radius);padding:24px;border:1px solid var(--gray-200);">
            ...
            <div style="margin-bottom:16px;">
                ...
                <textarea name="suspend_reason" class="form-control" required rows="4"
                          placeholder="Provide detailed reason for suspension..."
                          style="width:100%;padding:10px 14px;border:1px solid var(--gray-200);border-radius:10px;font-family:'Inter',sans-serif;font-size:0.85rem;resize:vertical;transition:var(--transition);"><?php echo htmlspecialchars($_POST['suspend_reason'] ?? ''); ?></textarea>
            ...
            <!-- Confirmation -->
            <div style="margin-bottom:16px;padding:12px 16px;background:#FEF2F2;border-radius:8px;border:1px solid #FECACA;">
                <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;">
                    ...
                    <span style="font-size:0.85rem;color:#991B1B;">
                        <strong>I confirm</strong> that I want to suspend this coordinator. I understand that this action will immediately revoke all access and prevent the coordinator from performing any actions.
                    </span>
                </label>
            </div>
            ...
            <div style="display:flex;gap:12px;margin-top:24px;padding-top:20px;border-top:1px solid var(--gray-200);flex-wrap:wrap;">
                <button type="submit" class="btn-danger" style="padding:10px 32px;background:#EF4444;color:white;border:none;border-radius:10px;font-family:'Inter',sans-serif;font-weight:600;font-size:0.85rem;cursor:pointer;transition:var(--transition);display:flex;align-items:center;gap:8px;">
                    <i class="fas fa-user-slash"></i> Suspend Coordinator
                </button>
