<?php
// ============================================================
// NATIONAL COORDINATOR - WARD DASHBOARD VIEW
// ============================================================
...
// If no Ward ID or invalid, redirect
if ($ward_id <= 0) {
    header('Location: monitor-states.php?error=invalid_ward');
    exit();
}
...
try {
    ...
    if (!$ward_data) {
        header('Location: monitor-states.php?error=ward_not_found');
        exit();
    }
    ...
    $state_id = $ward_data['state_id'];
    
} catch (Exception $e) {
    error_log("Ward Dashboard Check Error: " . $e->getMessage());
    header('Location: monitor-states.php?error=database_error');
    exit();
}

// ============================================================
// FETCH FULL WARD DATA WITH STATISTICS
// ============================================================
...
try {
    ...
    $ward_data['total_pus'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - PUs count error: " . $e->getMessage());
    $ward_data['total_pus'] = 0;
}

// Get Ward Coordinators count
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM users u JOIN roles r ON u.role_id = r.id WHERE u.tenant_id = ? AND r.level = 'ward' AND u.jurisdiction_id = ? AND u.status = 'active' AND (u.deleted_at IS NULL OR u.deleted_at = '0000-00-00 00:00:00')");
    $stmt->execute([$tenant_id, $ward_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $ward_data['ward_coordinators'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - Ward coordinators error: " . $e->getMessage());
    $ward_data['ward_coordinators'] = 0;
}

// Get PU Agents count
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM users u JOIN roles r ON u.role_id = r.id WHERE u.tenant_id = ? AND r.level = 'pu_agent' AND u.jurisdiction_id IN (SELECT id FROM polling_units WHERE ward_id = ?) AND u.status = 'active' AND (u.deleted_at IS NULL OR u.deleted_at = '0000-00-00 00:00:00')");
    $stmt->execute([$tenant_id, $ward_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $ward_data['pu_agents'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - PU agents error: " . $e->getMessage());
    $ward_data['pu_agents'] = 0;
}

// Get Elections count
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM elections WHERE tenant_id = ? AND deleted_at IS NULL AND wards_json IS NOT NULL AND wards_json != ''");
    $stmt->execute([$tenant_id]);
    $total_elections = $stmt->fetch(PDO::FETCH_ASSOC)['count'] ?? 0;
    
    if ($total_elections > 0) {
        $stmt = $db->prepare("SELECT COUNT(*) as count FROM elections WHERE tenant_id = ? AND deleted_at IS NULL AND wards_json IS NOT NULL AND wards_json != '' AND JSON_CONTAINS(wards_json, JSON_QUOTE(?))");
        $stmt->execute([$tenant_id, $ward_id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        $ward_data['election_count'] = $result['count'] ?? 0;
    } else {
        $ward_data['election_count'] = 0;
    }
} catch (Exception $e) {
    error_log("Ward Dashboard - Elections count error: " . $e->getMessage());
    $ward_data['election_count'] = 0;
}

// Get Active Elections
try {
    if (($ward_data['election_count'] ?? 0) > 0) {
        $stmt = $db->prepare("SELECT COUNT(*) as count FROM elections WHERE tenant_id = ? AND deleted_at IS NULL AND status = 'active' AND wards_json IS NOT NULL AND wards_json != '' AND JSON_CONTAINS(wards_json, JSON_QUOTE(?))");
        $stmt->execute([$tenant_id, $ward_id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        $ward_data['active_elections'] = $result['count'] ?? 0;
    } else {
        $ward_data['active_elections'] = 0;
    }
} catch (Exception $e) {
    error_log("Ward Dashboard - Active elections error: " . $e->getMessage());
    $ward_data['active_elections'] = 0;
}

// Get Total Results
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM results_ec8a r WHERE r.tenant_id = ? AND r.ward_id = ?");
    $stmt->execute([$tenant_id, $ward_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $ward_data['total_results'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - Total results error: " . $e->getMessage());
    $ward_data['total_results'] = 0;
}

// Get Verified Results
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM results_ec8a r WHERE r.tenant_id = ? AND r.ward_id = ? AND r.status = 'verified'");
    $stmt->execute([$tenant_id, $ward_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $ward_data['verified_results'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - Verified results error: " . $e->getMessage());
    $ward_data['verified_results'] = 0;
}

// Get Pending Results
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM results_ec8a r WHERE r.tenant_id = ? AND r.ward_id = ? AND r.status = 'pending'");
    $stmt->execute([$tenant_id, $ward_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $ward_data['pending_results'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - Pending results error: " . $e->getMessage());
    $ward_data['pending_results'] = 0;
}

// Get Flagged Results
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM results_ec8a r WHERE r.tenant_id = ? AND r.ward_id = ? AND r.status = 'flagged'");
    $stmt->execute([$tenant_id, $ward_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $ward_data['flagged_results'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - Flagged results error: " . $e->getMessage());
    $ward_data['flagged_results'] = 0;
}
...
try {
    ...
    $stmt->execute([$tenant_id, $ward_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $ward_data['ec8b_count'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - EC8B count error: " . $e->getMessage());
    $ward_data['ec8b_count'] = 0;
}

// Get Total Incidents
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM incidents WHERE tenant_id = ? AND ward_id = ?");
    $stmt->execute([$tenant_id, $ward_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $ward_data['total_incidents'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - Total incidents error: " . $e->getMessage());
    $ward_data['total_incidents'] = 0;
}

// Get Open Incidents
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM incidents WHERE tenant_id = ? AND ward_id = ? AND status IN ('reported', 'investigating')");
    $stmt->execute([$tenant_id, $ward_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $ward_data['open_incidents'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - Open incidents error: " . $e->getMessage());
    $ward_data['open_incidents'] = 0;
}

// Get Resolved Incidents
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM incidents WHERE tenant_id = ? AND ward_id = ? AND status = 'resolved'");
    $stmt->execute([$tenant_id, $ward_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $ward_data['resolved_incidents'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - Resolved incidents error: " . $e->getMessage());
    $ward_data['resolved_incidents'] = 0;
}

// Get Critical Incidents
try {
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM incidents WHERE tenant_id = ? AND ward_id = ? AND severity = 'critical'");
    $stmt->execute([$tenant_id, $ward_id]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $ward_data['critical_incidents'] = $result['count'] ?? 0;
} catch (Exception $e) {
    error_log("Ward Dashboard - Critical incidents error: " . $e->getMessage());
    $ward_data['critical_incidents'] = 0;
}
...
try {
    $stmt = $db->prepare("
        SELECT 
            ...
            pu.network_quality,
            pu.is_rural,
            (SELECT COUNT(*) FROM users u JOIN roles r ON u.role_id = r.id WHERE u.tenant_id = ? AND r.level = 'pu_agent' AND u.jurisdiction_id = pu.id AND u.status = 'active' AND (u.deleted_at IS NULL OR u.deleted_at = '0000-00-00 00:00:00')) as agents,
            (SELECT COUNT(*) FROM results_ec8a r WHERE r.tenant_id = ? AND r.pu_id = pu.id) as total_results,
            (SELECT COUNT(*) FROM results_ec8a r WHERE r.tenant_id = ? AND r.pu_id = pu.id AND r.status = 'verified') as verified_results,
            (SELECT COUNT(*) FROM results_ec8a r WHERE r.tenant_id = ? AND r.pu_id = pu.id AND r.status = 'pending') as pending_results,
    ...
    ");
    $stmt->execute([$tenant_id, $tenant_id, $tenant_id, $tenant_id, $tenant_id, $ward_id]);
    $polling_units = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    error_log("Ward Dashboard - PUs fetch error: " . $e->getMessage());
    $polling_units = [];
}
...
try {
    ...
    error_log("Ward Dashboard - Recent activities error: " . $e->getMessage());
    $recent_activities = [];
}
...
// CALCULATE PROGRESS
// ============================================================
$total_pus = $ward_data['total_pus'] ?? 0;
$verified_results = $ward_data['verified_results'] ?? 0;
    ...
    <div class="main-content-inner">
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-top:8px;">
                <div>
                    ...
                    <p style="color:var(--gray-500);margin:2px 0 0;">
                        <i class="fas fa-map-marker-alt"></i> <?php echo htmlspecialchars($lga_name); ?> LGA, <?php echo htmlspecialchars($state_name); ?> State
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:12px;">
                <div>
                    ...
                    <p style="font-size:0.75rem;color:var(--gray-500);margin:2px 0 0;">
                        <?php echo number_format($ward_data['verified_results'] ?? 0); ?> of <?php echo number_format($ward_data['total_pus'] ?? 0); ?> polling units verified
                    </p>
                </div>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-number"><?php echo number_format($ward_data['total_pus'] ?? 0); ?></div>
                <div class="stat-label">Polling Units</div>
                <div class="stat-change"><i class="fas fa-users"></i> <?php echo $ward_data['pu_agents'] ?? 0; ?> agents</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-icon purple"><i class="fas fa-user-tie"></i></div>
                <div class="stat-number"><?php echo number_format($ward_data['ward_coordinators'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-number"><?php echo number_format($ward_data['election_count'] ?? 0); ?></div>
                <div class="stat-label">Elections</div>
                <div class="stat-change up"><i class="fas fa-play"></i> <?php echo $ward_data['active_elections'] ?? 0; ?> active</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-icon yellow"><i class="fas fa-check-double"></i></div>
                <div class="stat-number"><?php echo number_format($ward_data['verified_results'] ?? 0); ?></div>
                <div class="stat-label">Verified Results</div>
                <div class="stat-change down"><i class="fas fa-clock"></i> <?php echo $ward_data['pending_results'] ?? 0; ?> pending</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-icon teal"><i class="fas fa-file-alt"></i></div>
                <div class="stat-number"><?php echo number_format($ward_data['ec8b_count'] ?? 0); ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-number"><?php echo number_format($ward_data['total_incidents'] ?? 0); ?></div>
                <div class="stat-label">Incidents</div>
                <div class="stat-change down"><i class="fas fa-clock"></i> <?php echo $ward_data['open_incidents'] ?? 0; ?> open</div>
            </div>
        ...
        <!-- Incident Summary -->
        <?php if (($ward_data['total_incidents'] ?? 0) > 0): ?>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                ...
                <div style="display:flex;gap:16px;flex-wrap:wrap;">
                    <div style="display:flex;align-items:center;gap:6px;font-size:0.8rem;">
                        <span class="badge badge-danger"><?php echo $ward_data['critical_incidents'] ?? 0; ?></span>
                        <span style="color:var(--gray-500);">Critical</span>
                    </div>
                    <div style="display:flex;align-items:center;gap:6px;font-size:0.8rem;">
                        <span class="badge badge-warning"><?php echo $ward_data['open_incidents'] ?? 0; ?></span>
                        <span style="color:var(--gray-500);">Open</span>
                    </div>
                    <div style="display:flex;align-items:center;gap:6px;font-size:0.8rem;">
                        <span class="badge badge-success"><?php echo $ward_data['resolved_incidents'] ?? 0; ?></span>
