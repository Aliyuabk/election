<?php
// ============================================================
// STATE COORDINATOR - WARD DETAILS
// ============================================================
...
try {
    // Get ward details
    $stmt = $db->prepare("
        SELECT w.*, l.name as lga_name, s.name as state_name
        FROM wards w
    ...
    // Get polling units in ward
    $stmt = $db->prepare("
        SELECT id, name, code, registered_voters, is_active, gps_lat, gps_lng
        FROM polling_units
    ...
    // Get statistics
    // Total PUs
    $stats['total_pus'] = count($polling_units);
    ...
    $stmt = $db->prepare("
        ...
        FROM results_ec8a r
        JOIN polling_units pu ON r.pu_id = pu.id
        WHERE pu.ward_id = ? AND r.tenant_id = ? AND r.status IN ('verified', 'approved')
    ");
    $stmt->execute([$ward_id, $tenant_id]);
    $stats['verified_pus'] = (int)($stmt->fetch(PDO::FETCH_ASSOC)['count'] ?? 0);
    
    // Total agents in ward
    $stmt = $db->prepare("
        SELECT COUNT(*) as count
        FROM users
        WHERE ward_id = ? AND deleted_at IS NULL AND status = 'active'
    ");
    $stmt->execute([$ward_id]);
    ...
    // Get ward coordinator
    $stmt = $db->prepare("
        SELECT u.id, u.first_name, u.last_name, u.email, u.phone
        FROM users u
        JOIN roles r ON u.role_id = r.id
        WHERE u.ward_id = ? AND r.level = 'ward' AND u.deleted_at IS NULL AND u.status = 'active'
    ");
    $stmt->execute([$ward_id]);
    $stats['coordinator'] = $stmt->fetch(PDO::FETCH_ASSOC);
...
.coordinator-info .avatar.blue { background: #3B82F6; }
.coordinator-info .avatar.green { background: #10B981; }
.coordinator-info .avatar.purple { background: #8B5CF6; }
.coordinator-info .avatar.orange { background: #F59E0B; }
    ...
    <div class="main-content-inner">
        ...
        <div class="page-header">
            <div>
                <h2>
                    ...
                    Ward Details
                    <small><?php echo htmlspecialchars($ward['name'] ?? 'Ward'); ?> - Local Government Area</small>
                </h2>
            </div>
            <div>
                <a href="lga-details.php?id=<?php echo $ward['lga_id'] ?? 0; ?>" class="btn-secondary-sm">
        ...
        <?php if ($ward): ?>
            ...
            <div class="ward-header">
                <div class="title"><?php echo htmlspecialchars($ward['name']); ?></div>
                <div class="subtitle">
                    <i class="fas fa-map-marker-alt"></i> <?php echo htmlspecialchars($ward['lga_name']); ?>
                    <span style="margin-left:12px;">
                        <i class="fas fa-flag"></i> <?php echo htmlspecialchars($ward['state_name']); ?>
                    </span>
                    <span style="margin-left:12px;">
                ...
                <div class="meta">
                    <span><i class="fas fa-code"></i> Code: <?php echo htmlspecialchars($ward['code'] ?? 'N/A'); ?></span>
                    <span><i class="fas fa-users"></i> Registered Voters: <?php echo number_format($ward['registered_voters'] ?? 0); ?></span>
                </div>
            </div>

            <!-- Coordinator Info -->
            <?php if ($stats['coordinator']): ?>
                <div class="coordinator-info" style="margin-bottom:20px;">
                    <?php 
                    $colors = ['blue', 'green', 'purple', 'orange'];
                    $color_idx = ($stats['coordinator']['id'] ?? 0) % 4;
                    $avatar_color = $colors[$color_idx];
                    $initials = strtoupper(substr($stats['coordinator']['first_name'] ?? '', 0, 1) . substr($stats['coordinator']['last_name'] ?? '', 0, 1));
                    ?>
                    ...
                    <div>
                        <div style="font-weight:600;font-size:0.9rem;">
                            <?php echo htmlspecialchars($stats['coordinator']['first_name'] . ' ' . $stats['coordinator']['last_name']); ?>
                        </div>
                        <div style="font-size:0.8rem;color:var(--gray-500);">
                            <i class="fas fa-envelope"></i> <?php echo htmlspecialchars($stats['coordinator']['email'] ?? ''); ?>
                            <span style="margin-left:12px;"><i class="fas fa-phone"></i> <?php echo htmlspecialchars($stats['coordinator']['phone'] ?? ''); ?></span>
                        </div>
                    </div>
                    <div style="margin-left:auto;font-size:0.7rem;color:var(--gray-400);">
                        <i class="fas fa-user-tie"></i> Ward Coordinator
                    </div>
                </div>
            <?php endif; ?>
                        ...
                        <?php if (count($polling_units) > 0): ?>
                            ...
                            <?php foreach ($polling_units as $pu): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="polling-unit-details.php?id=<?php echo $pu['id']; ?>" class="btn-action btn-view">
