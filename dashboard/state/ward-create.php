...
try {
    ...
    error_log("Create Ward Error: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            // Check if code already exists in this LGA
            $stmt = $db->prepare("SELECT id FROM wards WHERE lga_id = ? AND code = ?");
            $stmt->execute([$lga_id, $code]);
            if ($stmt->fetch()) {
                $error = 'Ward code already exists in this LGA';
            } else {
                $stmt = $db->prepare("
                    INSERT INTO wards (
                        lga_id, code, name, registered_voters,
                        gps_lat, gps_lng, is_active
                    ) VALUES (?, ?, ?, ?, ?, ?, ?)
                ");
                ...
                $log_stmt = $db->prepare("
                    ...
                    VALUES (?, ?, 'ward_created', ?, 'ward', ?, NOW())
                ");
                $log_stmt->execute([
                    $user_id,
                    $tenant_id,
                    "Created ward: $name in $lga_name",
                    $ward_id
                ]);
                ...
                // Redirect
                header("Location: manage-wards.php?lga=$lga_id&success=1");
                exit();
            }
        } catch (Exception $e) {
            $error = 'Failed to create ward: ' . $e->getMessage();
            error_log("Ward Create Error: " . $e->getMessage());
        }
    }
}
    ...
    <div class="main-content-inner">
        ...
        <div class="welcome-section">
            <div class="breadcrumb">
                ...
                <i class="fas fa-chevron-right" style="font-size:0.6rem;color:var(--gray-400);"></i>
                <span style="font-weight:600;color:var(--gray-800);">Create Ward</span>
            </div>
        ...
        <form method="POST" action="" style="background:white;border-radius:var(--radius);padding:24px;border:1px solid var(--gray-200);">
            ...
            <div style="display:flex;gap:12px;margin-top:24px;padding-top:20px;border-top:1px solid var(--gray-200);flex-wrap:wrap;">
                <button type="submit" class="btn-primary" style="padding:10px 32px;background:var(--primary);color:white;border:none;border-radius:10px;font-family:'Inter',sans-serif;font-weight:600;font-size:0.85rem;cursor:pointer;transition:var(--transition);display:flex;align-items:center;gap:8px;">
                    <i class="fas fa-plus"></i> Create Ward
                </button>
        ...
        <div style="background:#F0FDF4;border-radius:var(--radius);padding:16px 20px;margin-top:20px;border:1px solid #A7F3D0;">
            <h4 style="font-size:0.85rem;font-weight:600;color:#065F46;margin:0 0 8px;">
                <i class="fas fa-info-circle"></i> Ward Creation Tips
            </h4>
            <ul style="font-size:0.8rem;color:#065F46;margin:0;padding-left:20px;">
                <li>Each ward must have a unique code within the LGA</li>
                <li>The ward name should clearly identify the area</li>
                <li>Registered voters count should match official records</li>
                <li>GPS coordinates help verify the ward location</li>
                <li>Make sure the ward is set to "Active" to start operations</li>
            </ul>
        </div>
    </div>
</main>
