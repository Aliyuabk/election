<?php
// ============================================================
// STATE COORDINATOR - BROADCAST TO PU AGENTS
// ============================================================
...
// Get PU Agents with their details
$pu_agents = [];
try {
    $stmt = $db->prepare("
    ...
    <div class="main-content-inner">
        <div class="broadcast-container">
            ...
            <div class="welcome-section">
                <div>
                    <h1><i class="fas fa-flag-checkered"></i> Broadcast to PU Agents</h1>
                    <p class="subtitle">
                        <i class="fas fa-flag"></i> 
                        <?php echo htmlspecialchars($state_name); ?> State - Send messages to Polling Unit Agents
                    </p>
                </div>
                <div class="actions">
            ...
            <?php if (empty($pu_agents)): ?>
                <div class="alert alert-error" style="text-align:center;padding:30px;">
                    <i class="fas fa-flag-checkered" style="font-size:2rem;display:block;margin-bottom:8px;"></i>
                    <h4>No PU Agents Found</h4>
                    <p style="font-size:0.85rem;color:var(--gray-500);">No Polling Unit agents have been assigned in <?php echo htmlspecialchars($state_name); ?>.</p>
                </div>
            <?php else: ?>
                <form method="POST" action="" id="broadcastForm">
                    ...
                    <div class="form-card">
                        ...
                        <div class="filter-bar">
                            <select id="lgaFilter" onchange="filterAgents()">
                        ...
                        <div class="recipient-list" id="recipientList">
                            ...
                            <?php foreach ($pu_agents as $agent): ?>
                                <div class="recipient-item" data-lga="<?php echo $agent['lga_id']; ?>" 
                                    ...
                                    <span class="ward"><?php echo htmlspecialchars($agent['ward_name']); ?></span>
                                    <span class="lga">- <?php echo htmlspecialchars($agent['lga_name']); ?></span>
...
// Filter agents by LGA and search
function filterAgents() {
    var lgaFilter = document.getElementById('lgaFilter').value;
    var searchTerm = document.getElementById('searchInput').value.toLowerCase();
    var items = document.querySelectorAll('.recipient-item');
    var visible = 0;
    
    items.forEach(function(item) {
        var matchLga = !lgaFilter || item.dataset.lga === lgaFilter;
        var matchSearch = !searchTerm || item.dataset.search.includes(searchTerm);
        
        if (matchLga && matchSearch) {
            item.style.display = '';
            visible++;
        } else {
