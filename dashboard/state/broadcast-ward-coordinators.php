<?php
// ============================================================
// STATE COORDINATOR - BROADCAST TO WARD COORDINATORS
// ============================================================
...
try {
    $stmt = $db->prepare("
        SELECT u.id, u.first_name, u.last_name, u.email, u.phone, l.name as lga_name, w.name as ward_name
        FROM users u
        JOIN roles r ON u.role_id = r.id
        JOIN wards w ON u.ward_id = w.id
        JOIN lgas l ON w.lga_id = l.id
        WHERE r.level = 'ward' AND u.state_id = ? AND u.status = 'active' AND u.deleted_at IS NULL
        ORDER BY l.name ASC, w.name ASC, u.first_name ASC
    ");
    $stmt->execute([$state_id]);
    ...
    <div class="main-content-inner">
        <div class="broadcast-container">
            ...
            <div class="welcome-section">
                <div>
                    <h1><i class="fas fa-layer-group"></i> Broadcast to Ward Coordinators</h1>
                    <p class="subtitle">
                        <i class="fas fa-flag"></i> 
                        <?php echo htmlspecialchars($state_name); ?> State - Send messages to Ward Coordinators
                    </p>
                </div>
                <div class="actions">
            ...
            <?php if (empty($ward_coordinators)): ?>
                <div class="alert alert-error" style="text-align:center;padding:30px;">
                    ...
                    <p style="font-size:0.85rem;color:var(--gray-500);">No Ward coordinators have been assigned in <?php echo htmlspecialchars($state_name); ?>.</p>
                </div>
            <?php else: ?>
                <form method="POST" action="" id="broadcastForm">
                    ...
                    <div class="form-card">
                        ...
                        <div class="filter-bar">
                            <select id="lgaFilter" onchange="filterCoordinators()">
                        ...
                        <div class="recipient-list" id="recipientList">
                            ...
                            <?php foreach ($ward_coordinators as $coordinator): ?>
                                <div class="recipient-item" data-lga="<?php echo $coordinator['lga_id']; ?>">
...
function filterCoordinators() {
    ...
    items.forEach(function(item) {
        if (!lgaFilter || item.dataset.lga === lgaFilter) {
            item.style.display = '';
            visible++;
        } else {
