...
if (isset($_GET['action']) && $_GET['action'] === 'resend' && isset($_GET['id'])) {
    ...
    try {
        ...
        if ($broadcast) {
            ...
            if ($broadcast['target_audience'] === 'all') {
            ...
            } elseif ($broadcast['target_audience'] === 'lga' && !empty($target_ids)) {
                ...
                $recipients = $stmt->fetchAll(PDO::FETCH_ASSOC);
            } elseif ($broadcast['target_audience'] === 'role_specific' && !empty($target_ids)) {
                $placeholders = implode(',', array_fill(0, count($target_ids), '?'));
                $stmt = $db->prepare("
                    SELECT email, full_name FROM users u
                    WHERE u.tenant_id = ? AND u.status = 'active'
                        ...
                        $email_body = "
                            ...
                            <body>
                                <div class=\"container\">
                                    <div class=\"header\">
                                        <h1>📢 " . APP_NAME . "</h1>
                                        <p style=\"color: #64748B;\">Broadcast Message <span class=\"state-badge\">" . htmlspecialchars($state_name) . "</span> <span class=\"resend-badge\">🔁 Resent</span></p>
                                    </div>
...
try {
    ...
    $total_broadcasts = count($broadcasts);
} catch (Exception $e) {
    ...
    <div class="main-content-inner">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-number"><?php echo number_format($stats['scheduled']); ?></div>
        ...
        <div style="background:white;border-radius:var(--radius);overflow:hidden;border:1px solid var(--gray-200);">
            <div style="padding:12px 20px;border-bottom:1px solid var(--gray-200);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                ...
                <div style="display:flex;gap:8px;align-items:center;">
                    <span style="font-size:0.75rem;color:var(--gray-500);"><?php echo number_format($stats['total']); ?> broadcasts</span>
        ...
        <div style="background:#F8FAFC;border-radius:var(--radius);padding:12px 20px;margin-top:16px;border:1px solid var(--gray-200);display:flex;flex-wrap:wrap;gap:16px;justify-content:space-between;align-items:center;">
            <div style="display:flex;gap:20px;flex-wrap:wrap;font-size:0.75rem;color:var(--gray-500);">
                <span><i class="fas fa-envelope" style="color:var(--primary);"></i> Total: <strong><?php echo number_format($stats['total']); ?></strong></span>
                <span><i class="fas fa-check-circle" style="color:#10B981;"></i> Sent: <strong><?php echo number_format($stats['sent']); ?></strong></span>
                <span><i class="fas fa-clock" style="color:#F59E0B;"></i> Scheduled: <strong><?php echo number_format($stats['scheduled']); ?></strong></span>
                <span><i class="fas fa-pencil-alt" style="color:#8B5CF6;"></i> Drafts: <strong><?php echo number_format($stats['draft']); ?></strong></span>
            </div>
