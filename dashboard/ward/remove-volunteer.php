<?php
// ============================================================
// WARD COORDINATOR - REMOVE VOLUNTEER
// ============================================================
...
if ($volunteer_id <= 0) {
    header('Location: assign-volunteers.php');
    exit();
}
...
try {
    $stmt = $db->prepare("
        SELECT 
            ...
            pu.name as pu_name,
            pu.code as pu_code,
            (SELECT COUNT(*) FROM volunteer_tasks vt WHERE vt.volunteer_id = u.id) as total_tasks,
            (SELECT COUNT(*) FROM volunteer_tasks vt WHERE vt.volunteer_id = u.id AND vt.status = 'completed') as completed_tasks
        FROM users u
    ...
    if (!$volunteer) {
        header('Location: assign-volunteers.php?error=notfound');
        exit();
    }
    
    if (empty($volunteer['pu_id'])) {
        header('Location: assign-volunteers.php?error=not_assigned');
        exit();
    }
    
} catch (Exception $e) {
    error_log("Error fetching volunteer: " . $e->getMessage());
    header('Location: assign-volunteers.php?error=db');
    exit();
}
    ...
    if ($action === 'remove') {
        if (empty($reason)) {
            ...
            try {
                ...
                logActivity($user_id, 'volunteer_removed', "Removed volunteer ID: $volunteer_id from PU - Reason: $reason", 'user', $volunteer_id);
                
                $db->commit();
                $success_message = "Volunteer assignment removed successfully!";
                header('Location: assign-volunteers.php?success=' . urlencode($success_message));
                exit();
                
            } catch (Exception $e) {
                $db->rollBack();
                $error_message = "Error removing volunteer: " . $e->getMessage();
                error_log("Volunteer removal error: " . $e->getMessage());
            }
        }
    }
}

$page_title = 'Remove Volunteer';
