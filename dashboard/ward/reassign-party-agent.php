<?php
// ============================================================
// WARD COORDINATOR - REASSIGN PARTY AGENT
// ============================================================
...
if ($agent_id <= 0) {
    header('Location: assign-party-agents.php');
    exit();
}
...
try {
    ...
    if (!$agent) {
        header('Location: assign-party-agents.php?error=notfound');
        exit();
    }
    
    if (empty($agent['pu_id'])) {
        header('Location: assign-party-agents.php?error=not_assigned');
        exit();
    }
    ...
    header('Location: assign-party-agents.php?error=db');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($agent_id > 0 && $new_pu_id > 0) {
        try {
            ...
            $stmt->execute([$new_pu_id, $agent_id, $tenant_id]);
            ...
            logActivity($user_id, 'party_agent_reassigned', "Reassigned party agent ID: $agent_id to PU: $new_pu_id", 'user', $agent_id);
            
            $db->commit();
            $success_message = "Party agent reassigned successfully!";
            header('Location: assign-party-agents.php?success=' . urlencode($success_message));
            exit();
            
        } catch (Exception $e) {
            $db->rollBack();
            $error_message = "Error reassigning party agent: " . $e->getMessage();
            error_log("Party agent reassign error: " . $e->getMessage());
        }
    } else {
...
$page_title = 'Reassign Party Agent';
