...
function updateBrandPreview() {
    ...
    if (preview) {
        ...
        const content = preview.querySelector('.preview-content p');
        const icon = preview.querySelector('.preview-content p i');
        
        if (header) header.style.backgroundColor = primary;
        if (badge) badge.style.backgroundColor = secondary;
        if (content) content.style.color = primary;
        if (icon) icon.style.color = secondary;
    }
}
...
function updatePlanDetails() {
    ...
    details.innerHTML = `
        ...
        <p style="font-size:1.2rem; font-weight:600; color:#0b1a33;">${planData.price}</p>
...
// Initialize plan details on load
document.addEventListener('DOMContentLoaded', function() {
    updatePlanDetails();
});
...
function deleteTenant(id) {
    if (confirm('⚠️ Are you sure you want to permanently delete this tenant?\n\nThis action cannot be undone and will remove all associated data including users, elections, and settings.')) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = 'tenants.php';
        form.innerHTML = `
...
// ADD SUBSCRIPTION
// ============================================================
function addSubscription(tenantId) {
    window.location.href = `tenant-subscriptions.php?tenant_id=${tenantId}`;
}
