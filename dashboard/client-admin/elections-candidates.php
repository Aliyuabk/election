<?php
// ============================================================
// ELECTION CANDIDATES - CLIENT ADMIN (PROFESSIONAL)
// ============================================================
...
$election = null;
try {
    $stmt = $db->prepare("SELECT id, name, type, tenant_id FROM elections WHERE id = ? AND tenant_id = ? AND deleted_at IS NULL");
    ...
    try {
        switch ($action) {
            case 'add_candidate':
                ...
                logActivity($user_id, 'candidate_added', "Added candidate: $first_name $last_name to election ID: $election_id");
            ...
            case 'edit_candidate':
                ...
                $stmt = $db->prepare("
                    UPDATE candidates SET
                        ...
                        biography = ?,
                        is_active = ?
                    WHERE id = ? AND election_id = ? AND tenant_id = ?
                ");
                $stmt->execute([
                    $first_name,
                    $last_name,
            ...
            case 'delete_candidate':
                ...
                $stmt = $db->prepare("DELETE FROM candidates WHERE id = ? AND election_id = ? AND tenant_id = ?");
                $stmt->execute([$id, $election_id, $tenant_id]);
...
<style>
    /* ============================================================
       ELECTION CANDIDATES - CLIENT ADMIN STYLES
       ============================================================ */
            ...
            <div class="info">
                ...
                <div>
                    ...
                    <div class="meta">
                        ...
                        <span><i class="fas fa-user-tie"></i> <?php echo count($candidates); ?> candidates</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="filter-bar">
            <form method="GET" action="" style="display:flex;flex-wrap:wrap;gap:10px;flex:1;align-items:center;width:100%;">
                ...
                <?php if (!empty($_GET['search'])): ?>
                    <a href="elections-candidates.php?id=<?php echo $election_id; ?>" class="btn-clear"><i class="fas fa-times"></i> Clear</a>
                <?php endif; ?>
            </form>
        </div>
                            ...
                            <tr>
                                    ...
                                    <div style="display:flex;align-items:center;gap:10px;">
                                        <div class="candidate-avatar <?php echo $avatar_color; ?>">
                                            <?php if (!empty($candidate['photograph_url'])): ?>
                                                <img src="<?php echo htmlspecialchars($candidate['photograph_url']); ?>" alt="Photo">
                                            <?php else: ?>
                                ...
                                <td>
                                    <?php if (!empty($candidate['party_name'])): ?>
                                        <span class="party-tag <?php echo !empty($candidate['party_logo']) ? 'has-logo' : ''; ?>">
                                            <?php if (!empty($candidate['party_logo'])): ?>
                                                <img src="<?php echo htmlspecialchars($candidate['party_logo']); ?>" alt="Logo" class="party-logo-sm">
                                            <?php endif; ?>
...
<div class="modal-overlay" id="editCandidateModal">
    <div class="modal">
        ...
        <form method="POST" action="" id="editCandidateForm">
            ...
            <div class="form-group">
                <label>Biography</label>
                <textarea name="biography" id="editBiography" placeholder="Brief biography..."></textarea>
            </div>
            <div class="form-group">
                <div style="display:flex;align-items:center;gap:10px;">
