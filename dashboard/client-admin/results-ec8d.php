<?php
// ============================================================
// EC8D RESULTS - STATE LEVEL (PROFESSIONAL UI)
// ============================================================
    ...
    try {
        switch ($action) {
            case 'verify_result':
                ...
                $stmt = $db->prepare("UPDATE results_ec8d SET status = 'verified', verified_by = ? WHERE id = ? AND tenant_id = ?");
                $stmt->execute([$user_id, $id, $tenant_id]);
                
                logActivity($user_id, 'ec8d_verified', "Verified EC8D result ID: $id");
            ...
            case 'declare_result':
                ...
                $stmt = $db->prepare("UPDATE results_ec8d SET status = 'declared', verified_by = ? WHERE id = ? AND tenant_id = ?");
                $stmt->execute([$user_id, $id, $tenant_id]);
                
                logActivity($user_id, 'ec8d_declared', "Declared EC8D result ID: $id");
                $action_result = ['success' => true, 'message' => 'Result declared successfully.'];
                break;
            ...
            case 'delete_result':
                ...
                $stmt = $db->prepare("DELETE FROM results_ec8d WHERE id = ? AND tenant_id = ?");
                $stmt->execute([$id, $tenant_id]);
                
                logActivity($user_id, 'ec8d_deleted', "Deleted EC8D result ID: $id");
...
if (!empty($search)) {
    $where_conditions[] = "(s.name LIKE ? OR s.code LIKE ?)";
    $search_param = "%$search%";
...
// Count total
$count_sql = "SELECT COUNT(*) as total FROM results_ec8d r $where_clause";
...
try {
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8d WHERE tenant_id = ?");
    $stmt->execute([$tenant_id]);
    $stats['total'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8d WHERE tenant_id = ? AND status = 'verified'");
    $stmt->execute([$tenant_id]);
    $stats['verified'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8d WHERE tenant_id = ? AND status = 'pending'");
    $stmt->execute([$tenant_id]);
    $stats['pending'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8d WHERE tenant_id = ? AND status = 'declared'");
    $stmt->execute([$tenant_id]);
    $stats['declared'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8d WHERE tenant_id = ? AND status = 'rejected'");
    $stmt->execute([$tenant_id]);
    $stats['rejected'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8d WHERE tenant_id = ? AND status = 'flagged'");
    $stmt->execute([$tenant_id]);
    $stats['flagged'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT SUM(valid_votes) as total FROM results_ec8d WHERE tenant_id = ?");
    $stmt->execute([$tenant_id]);
    $stats['total_votes'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8d WHERE tenant_id = ? AND mismatch_alert = 1");
    $stmt->execute([$tenant_id]);
...
?>
<style>
    /* Same styles as EC8C with state-level specific colors */
    .results-nav {
        display: flex;
        gap: 6px;
        flex-wrap: wrap;
    ...
    .badge-status.declared { background: #EFF6FF; color: #1E40AF; border: 1px solid #93C5FD; }
    .badge-status.declared .dot { background: #3B82F6; }
...
<main class="main-content">
    ...
    <div class="main-content-inner">
        ...
        <div class="page-header">
            <div>
                <h2>
                    <i class="fas fa-flag" style="color:var(--primary);margin-right:8px;"></i> EC8D - State Level Results
                    <small>Manage and declare state level consolidated results</small>
                </h2>
            </div>
        ...
        <div class="results-nav">
            ...
            <a href="results-ec8d.php" class="active">
        ...
        <div class="stats-grid">
            <div class="stat-item">
                ...
                <div class="sub-label">State level</div>
            </div>
            <div class="stat-item">
            ...
            <div class="stat-item">
                <div class="number blue"><?php echo number_format($stats['declared']); ?></div>
        ...
        <div class="filter-bar">
            <form method="GET" action="" style="display:flex;flex-wrap:wrap;gap:10px;flex:1;align-items:center;width:100%;">
                <div class="search-wrap" style="flex:1;">
                    ...
                    <input type="text" name="search" placeholder="Search by state name or code..." value="<?php echo htmlspecialchars($search); ?>">
                </div>
                ...
                <?php if (!empty($search) || $election_filter > 0 || !empty($status_filter)): ?>
                    <a href="results-ec8d.php" class="btn-clear"><i class="fas fa-times"></i> Clear</a>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div class="table-container">
            <div class="table-header">
                <div class="table-title">
                    <i class="fas fa-list" style="color:var(--primary);"></i> EC8D Results List
                                ...
                                <td>
                                    <div>
                                        ...
                                        <div style="font-size:0.65rem;color:var(--gray-400);">
                                            Code: <?php echo htmlspecialchars($result['state_code'] ?? 'N/A'); ?>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    ...
                                    <div class="action-dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <?php if ($result['status'] == 'pending' || $result['status'] == 'verified'): ?>
                                                <button onclick="declareResult(<?php echo $result['id']; ?>)">
                                                    <i class="fas fa-gavel" style="color:#3B82F6;"></i> Declare
                                                </button>
                                            <?php endif; ?>
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="empty-state">
                                    ...
                                    <p>EC8D results will appear here once they are submitted.</p>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?>
...
<script>
// Same JavaScript as EC8C with additional declare function
...
function declareResult(id) {
    if (confirm('Are you sure you want to officially declare this result? This action is final and cannot be undone.')) {
        var form = document.createElement('form');
        form.method = 'POST';
...
function flagResult(id) {
    if (confirm('Flag this result for review?')) {
        var form = document.createElement('form');
        form.method = 'POST';
