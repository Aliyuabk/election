<?php
// ============================================================
// EC8E RESULTS - NATIONAL LEVEL (PROFESSIONAL UI)
// ============================================================
    ...
    try {
        switch ($action) {
            case 'verify_result':
                ...
                $stmt = $db->prepare("UPDATE results_ec8e SET status = 'verified', verified_by = ? WHERE id = ? AND tenant_id = ?");
                $stmt->execute([$user_id, $id, $tenant_id]);
                
                logActivity($user_id, 'ec8e_verified', "Verified EC8E result ID: $id");
            ...
            case 'declare_result':
                ...
                $stmt = $db->prepare("UPDATE results_ec8e SET status = 'declared', verified_by = ?, declaration_time = NOW() WHERE id = ? AND tenant_id = ?");
                $stmt->execute([$user_id, $id, $tenant_id]);
                
                logActivity($user_id, 'ec8e_declared', "Declared EC8E result ID: $id");
                $action_result = ['success' => true, 'message' => 'National result declared successfully!'];
                break;
            ...
            case 'delete_result':
                ...
                $stmt = $db->prepare("DELETE FROM results_ec8e WHERE id = ? AND tenant_id = ?");
                $stmt->execute([$id, $tenant_id]);
                
                logActivity($user_id, 'ec8e_deleted', "Deleted EC8E result ID: $id");
...
// Count total
$count_sql = "SELECT COUNT(*) as total FROM results_ec8e r $where_clause";
...
try {
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8e WHERE tenant_id = ?");
    $stmt->execute([$tenant_id]);
    $stats['total'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8e WHERE tenant_id = ? AND status = 'verified'");
    $stmt->execute([$tenant_id]);
    $stats['verified'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8e WHERE tenant_id = ? AND status = 'pending'");
    $stmt->execute([$tenant_id]);
    $stats['pending'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8e WHERE tenant_id = ? AND status = 'declared'");
    $stmt->execute([$tenant_id]);
    $stats['declared'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8e WHERE tenant_id = ? AND status = 'rejected'");
    $stmt->execute([$tenant_id]);
    $stats['rejected'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8e WHERE tenant_id = ? AND status = 'flagged'");
    $stmt->execute([$tenant_id]);
    $stats['flagged'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT SUM(valid_votes) as total FROM results_ec8e WHERE tenant_id = ?");
    $stmt->execute([$tenant_id]);
    $stats['total_votes'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8e WHERE tenant_id = ? AND mismatch_alert = 1");
    $stmt->execute([$tenant_id]);
    $stats['mismatch_alerts'] = $stmt->fetch()['total'] ?? 0;
} catch (Exception $e) {}
...
<style>
    /* ============================================================
       EC8E NATIONAL RESULTS - PROFESSIONAL UI STYLES
       ============================================================ */
    ...
    .stat-item .number.gold { color: #D4AF37; }
    .stat-item .label {
        font-size: 0.75rem;
        color: var(--gray-500);
        margin-top: 4px;
    ...
    .badge-status.declared { background: #F5F3FF; color: #5B21B6; border: 1px solid #C4B5FD; }
    .badge-status.declared .dot { background: #8B5CF6; }
    ...
    .badge-mismatch .icon { font-size: 0.6rem; }
    .badge-mismatch.clear {
        background: #ECFDF5;
        color: #065F46;
        border-color: #A7F3D0;
    }
    ...
    .declaration-banner .info {
        display: flex;
        align-items: center;
        gap: 16px;
    }
    .declaration-banner .info .icon {
        font-size: 2rem;
        color: #FCD34D;
    }
    .declaration-banner .info h3 {
        font-weight: 700;
        font-size: 1.1rem;
        margin-bottom: 2px;
    }
    .declaration-banner .info p {
        font-size: 0.85rem;
        color: rgba(255,255,255,0.7);
    }
    .declaration-banner .actions {
        display: flex;
        gap: 10px;
    }
    ...
    @media (max-width: 768px) {
        ...
        .declaration-banner { flex-direction: column; align-items: flex-start; }
        .declaration-banner .actions { width: 100%; }
        .declaration-banner .actions .btn { flex: 1; justify-content: center; }
    }
...
<main class="main-content">
    ...
    <div class="main-content-inner">
        ...
        <div class="page-header">
            <div>
                <h2>
                    <i class="fas fa-globe-africa" style="color:var(--primary);margin-right:8px;"></i> EC8E - National Level Results
                    <small>Manage and declare national level final results</small>
                </h2>
            </div>
        ...
        <div class="results-nav">
            ...
            <a href="results-ec8e.php" class="active">
        ...
        <div class="declaration-banner">
            <div class="info">
                ...
                <div>
                    <h3>National Results Declared</h3>
                    <p>Official national results have been declared for <?php echo number_format($stats['declared']); ?> election(s)</p>
                </div>
            </div>
            <div class="actions">
                <a href="#" class="btn-success" style="padding:8px 16px;font-size:0.8rem;text-decoration:none;">
                    <i class="fas fa-file-alt"></i> View Declaration
                </a>
                <a href="#" class="btn-outline" style="padding:8px 16px;font-size:0.8rem;background:rgba(255,255,255,0.1);border-color:rgba(255,255,255,0.2);color:white;">
                    <i class="fas fa-print"></i> Print
                </a>
            </div>
        </div>
        <?php else: ?>
        <div class="declaration-banner" style="background:linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%);">
            <div class="info">
                <div class="icon">
                    <i class="fas fa-hourglass-half"></i>
                </div>
                <div>
                    <h3>Awaiting National Declaration</h3>
                    <p>National results are pending declaration. Review and verify all state results first.</p>
                </div>
            </div>
            <div class="actions">
                <?php if ($stats['pending'] > 0 || $stats['flagged'] > 0): ?>
                    <span style="font-size:0.8rem;color:#FCD34D;padding:8px 16px;background:rgba(255,255,255,0.1);border-radius:8px;">
                        <i class="fas fa-exclamation-triangle"></i> 
                        <?php echo number_format($stats['pending'] + $stats['flagged']); ?> result(s) need attention
                    </span>
                <?php else: ?>
                    <button onclick="declareNationalResult()" class="btn-success" style="padding:8px 16px;font-size:0.8rem;border:none;border-radius:8px;">
                        <i class="fas fa-gavel"></i> Declare National Results
                    </button>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="stats-grid">
            <div class="stat-item">
                ...
                <div class="sub-label">National level</div>
            </div>
            <div class="stat-item">
            ...
            <div class="stat-item">
                <div class="number gold"><?php echo number_format($stats['declared']); ?></div>
                <div class="label">Declared</div>
                <div class="sub-label">Officially declared</div>
            </div>
            <div class="stat-item">
            ...
            <div class="stat-item">
                <div class="number orange"><?php echo number_format($stats['mismatch_alerts']); ?></div>
                <div class="label">Mismatch Alerts</div>
                <div class="sub-label">Needs investigation</div>
            </div>
            <div class="stat-item">
                <div class="number purple"><?php echo number_format($stats['total_votes']); ?></div>
        ...
        <div class="filter-bar">
            <form method="GET" action="" style="display:flex;flex-wrap:wrap;gap:10px;flex:1;align-items:center;width:100%;">
                <div class="search-wrap" style="flex:1;">
                    ...
                    <input type="text" name="search" placeholder="Search by election name..." value="<?php echo htmlspecialchars($search); ?>">
                </div>
                ...
                <select name="status">
                    ...
                    <option value="declared" <?php echo $status_filter == 'declared' ? 'selected' : ''; ?>>Declared</option>
                    <option value="rejected" <?php echo $status_filter == 'rejected' ? 'selected' : ''; ?>>Rejected</option>
                    <option value="flagged" <?php echo $status_filter == 'flagged' ? 'selected' : ''; ?>>Flagged</option>
                </select>
                <button type="submit" class="btn-filter"><i class="fas fa-filter"></i> Filter</button>
                <?php if (!empty($search) || $election_filter > 0 || !empty($status_filter)): ?>
                    <a href="results-ec8e.php" class="btn-clear"><i class="fas fa-times"></i> Clear</a>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div class="table-container">
            <div class="table-header">
                <div class="table-title">
                    <i class="fas fa-list" style="color:var(--primary);"></i> EC8E Results List
                    ...
                    <?php if (count($results) > 0): ?>
                        <?php foreach ($results as $index => $result): ?>
                            <tr>
                                ...
                                <td>
                                    <?php if ($result['mismatch_alert'] ?? 0): ?>
                                        <span class="badge-mismatch">
                                            <i class="fas fa-exclamation-triangle icon"></i> Alert
                                        </span>
                                    <?php else: ?>
                                        <span class="badge-mismatch clear">
                                            <i class="fas fa-check-circle icon"></i> Clear
                                        </span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    ...
                                    <div class="action-dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <?php if ($result['status'] == 'pending' || $result['status'] == 'verified'): ?>
                                                <button onclick="declareNationalResultFromTable(<?php echo $result['id']; ?>)">
                                                    <i class="fas fa-gavel" style="color:#8B5CF6;"></i> Declare
                                                </button>
                                            <?php endif; ?>
                        ...
                        <tr>
                            <td colspan="6">
                                <div class="empty-state">
                                    ...
                                    <h4>No national results found</h4>
                                    <p>EC8E results will appear here once they are submitted.</p>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?>
        ...
        <div class="pagination">
            ...
            <div class="pages">
                <?php if ($page > 1): ?>
                    <a href="?page=<?php echo $page - 1; ?>&search=<?php echo urlencode($search); ?>&election=<?php echo $election_filter; ?>&status=<?php echo urlencode($status_filter); ?>">
                ...
                if ($start_page > 1) {
                    echo '<a href="?page=1&search=' . urlencode($search) . '&election=' . $election_filter . '&status=' . urlencode($status_filter) . '">1</a>';
                    if ($start_page > 2) echo '<span>…</span>';
                }
                for ($i = $start_page; $i <= $end_page; $i++):
                ?>
                    <a href="?page=<?php echo $i; ?>&search=<?php echo urlencode($search); ?>&election=<?php echo $election_filter; ?>&status=<?php echo urlencode($status_filter); ?>" class="<?php echo $i === $page ? 'active' : ''; ?>">
                        <?php echo $i; ?>
                    </a>
                <?php endfor;
                if ($end_page < $total_pages) {
                    if ($end_page < $total_pages - 1) echo '<span>…</span>';
                    echo '<a href="?page=' . $total_pages . '&search=' . urlencode($search) . '&election=' . $election_filter . '&status=' . urlencode($status_filter) . '">' . $total_pages . '</a>';
                }
                ?>
...
// ============================================================
// NATIONAL RESULT FUNCTIONS
// ============================================================
function viewResultDetails(id) {
    alert('View details for national result ID: ' + id);
}

function verifyResult(id) {
    if (confirm('Are you sure you want to verify this national result?')) {
...
function declareNationalResultFromTable(id) {
    if (confirm('Are you sure you want to officially declare this national result? This is the final step and cannot be undone.')) {
        var form = document.createElement('form');
        form.method = 'POST';
        form.innerHTML = '<input type="hidden" name="action" value="declare_result"><input type="hidden" name="id" value="' + id + '">';
        document.body.appendChild(form);
        form.submit();
    }
}

function declareNationalResult() {
    if (confirm('Are you sure you want to officially declare the national results? This is the final declaration and cannot be undone.')) {
        // In production, this would declare the most recent pending result
        alert('National results declared successfully!');
...
function flagResult(id) {
    if (confirm('Flag this national result for review?')) {
        var form = document.createElement('form');
        form.method = 'POST';
...
function deleteResult(id) {
    if (confirm('Delete this national result? This action cannot be undone.')) {
        var form = document.createElement('form');
        form.method = 'POST';
