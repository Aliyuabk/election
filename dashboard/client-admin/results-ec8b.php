<?php
// ============================================================
// EC8B RESULTS - WARD LEVEL (PROFESSIONAL UI)
// ============================================================
    ...
    try {
        switch ($action) {
            case 'verify_result':
                ...
                $stmt = $db->prepare("UPDATE results_ec8b SET status = 'verified', verified_by = ? WHERE id = ? AND tenant_id = ?");
                $stmt->execute([$user_id, $id, $tenant_id]);
                
                logActivity($user_id, 'ec8b_verified', "Verified EC8B result ID: $id");
            ...
            case 'reject_result':
                ...
                $stmt = $db->prepare("UPDATE results_ec8b SET status = 'rejected' WHERE id = ? AND tenant_id = ?");
                $stmt->execute([$id, $tenant_id]);
                
                logActivity($user_id, 'ec8b_rejected', "Rejected EC8B result ID: $id");
            ...
            case 'flag_result':
                ...
                $stmt = $db->prepare("UPDATE results_ec8b SET status = 'flagged' WHERE id = ? AND tenant_id = ?");
                $stmt->execute([$id, $tenant_id]);
                
                logActivity($user_id, 'ec8b_flagged', "Flagged EC8B result ID: $id");
            ...
            case 'delete_result':
                ...
                $stmt = $db->prepare("DELETE FROM results_ec8b WHERE id = ? AND tenant_id = ?");
                $stmt->execute([$id, $tenant_id]);
                
                logActivity($user_id, 'ec8b_deleted', "Deleted EC8B result ID: $id");
...
// Count total
$count_sql = "SELECT COUNT(*) as total FROM results_ec8b r $where_clause";
...
try {
    ...
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8b WHERE tenant_id = ? AND status = 'verified'");
    $stmt->execute([$tenant_id]);
    $stats['verified'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8b WHERE tenant_id = ? AND status = 'pending'");
    $stmt->execute([$tenant_id]);
    $stats['pending'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8b WHERE tenant_id = ? AND status = 'rejected'");
    $stmt->execute([$tenant_id]);
    $stats['rejected'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8b WHERE tenant_id = ? AND status = 'flagged'");
    $stmt->execute([$tenant_id]);
    $stats['flagged'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT SUM(valid_votes) as total FROM results_ec8b WHERE tenant_id = ?");
    $stmt->execute([$tenant_id]);
    $stats['total_votes'] = $stmt->fetch()['total'] ?? 0;
    
    $stmt = $db->prepare("SELECT COUNT(*) as total FROM results_ec8b WHERE tenant_id = ? AND mismatch_alert = 1");
    $stmt->execute([$tenant_id]);
...
<style>
    /* ============================================================
       EC8B RESULTS - PROFESSIONAL UI STYLES
       ============================================================ */
...
<main class="main-content">
    ...
    <div class="main-content-inner">
        ...
        <div class="page-header">
            <div>
                <h2>
                    <i class="fas fa-layer-group" style="color:var(--primary);margin-right:8px;"></i> EC8B - Ward Level Results
                    <small>Manage and verify ward level consolidated results</small>
                </h2>
            </div>
        ...
        <div class="results-nav">
            <a href="results-ec8a.php">
                <i class="fas fa-flag-checkered"></i> EC8A (PU)
            </a>
            <a href="results-ec8b.php" class="active">
        ...
        <div class="stats-grid">
            <div class="stat-item">
                ...
                <div class="sub-label">Ward level</div>
            </div>
            <div class="stat-item">
        ...
        <div class="filter-bar">
            <form method="GET" action="" style="display:flex;flex-wrap:wrap;gap:10px;flex:1;align-items:center;width:100%;">
                <div class="search-wrap" style="flex:1;">
                    ...
                    <input type="text" name="search" placeholder="Search by ward name or code..." value="<?php echo htmlspecialchars($search); ?>">
                </div>
                ...
                <?php if (!empty($search) || $election_filter > 0 || !empty($status_filter) || $state_filter > 0): ?>
                    <a href="results-ec8b.php" class="btn-clear"><i class="fas fa-times"></i> Clear</a>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div class="table-container">
            <div class="table-header">
                <div class="table-title">
                    <i class="fas fa-list" style="color:var(--primary);"></i> EC8B Results List
                                ...
                                <td>
                                    <div>
                                        <div style="font-weight:500;font-size:0.85rem;">
                                            <?php echo htmlspecialchars($result['ward_name'] ?? 'N/A'); ?>
                                        </div>
                                        <div style="font-size:0.65rem;color:var(--gray-400);">
                                            Code: <?php echo htmlspecialchars($result['ward_code'] ?? 'N/A'); ?>
                                        </div>
                                    </div>
                                </td>
                                <td>
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    ...
                                    <p>EC8B results will appear here once they are submitted.</p>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?>
