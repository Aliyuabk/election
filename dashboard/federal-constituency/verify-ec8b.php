<?php
// ============================================================
// FEDERAL CONSTITUENCY COORDINATOR - VERIFY EC8B
// ============================================================
...
try {
    ...
    if (!empty($search)) {
        $where[] = "(w.name LIKE ? OR l.name LIKE ?)";
        $params[] = "%$search%";
        $params[] = "%$search%";
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        if ($action === 'approve') {
            ...
            $action_success = 'EC8B approved successfully!';
            logActivity($user_id, 'ec8b_verified', "Verified EC8B result ID: $result_id", 'results_ec8b', $result_id);
            ...
            ");
            $stmt->execute([$user_id, $remarks, $result_id, $tenant_id]);
            $action_success = 'EC8B rejected.';
            logActivity($user_id, 'ec8b_rejected', "Rejected EC8B result ID: $result_id", 'results_ec8b', $result_id);
        } elseif ($action === 'flag') {
            $stmt = $db->prepare("
                UPDATE results_ec8b SET 
                    status = 'flagged',
            ...
            ");
            $stmt->execute([$user_id, $remarks, $result_id, $tenant_id]);
            $action_success = 'EC8B flagged for review.';
            logActivity($user_id, 'ec8b_flagged', "Flagged EC8B result ID: $result_id", 'results_ec8b', $result_id);
        }
    } catch (Exception $e) {
...
.submission-card .submission-header .ward-info {
    font-weight: 600;
    font-size: 0.95rem;
}
...
<main class="main-content">
    ...
    <div class="main-content-inner">
        <h2 style="font-size:1.3rem;font-weight:700;margin-bottom:4px;">
            <i class="fas fa-check-double" style="color:var(--primary);"></i> Verify EC8B Submissions
        </h2>
        <p style="color:var(--gray-500);font-size:0.9rem;margin-bottom:20px;">
            Review and verify Ward result submissions.
        </p>
        ...
        <div class="filter-section">
            <form method="GET" style="display:flex;gap:12px;flex-wrap:wrap;flex:1;align-items:center;">
                ...
                <input type="text" name="search" placeholder="Search ward..." value="<?php echo htmlspecialchars($search); ?>">
            ...
            <?php foreach ($submissions as $sub): ?>
                <div class="submission-card">
                    ...
                    <div class="submission-details">
                        ...
                        <div class="detail">
                            <div class="number"><?php echo number_format($sub['total_votes'] ?? 0); ?></div>
                            <div class="label">Total Votes</div>
                        </div>
                        <?php if ($sub['mismatch_alert'] ?? 0): ?>
                            <div class="detail" style="grid-column:1/-1;background:#FEE2E2;padding:8px;border-radius:6px;">
                                <div style="color:#DC2626;font-weight:600;font-size:0.8rem;">
                                    <i class="fas fa-exclamation-triangle"></i> Mismatch Alert
                                </div>
                            </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <?php if ($sub['status'] === 'pending'): ?>
                        <div class="submission-actions">
                            ...
                            <a href="ec8b-details.php?id=<?php echo $sub['id']; ?>" class="btn btn-view">
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <p>No EC8B submissions match your filter criteria.</p>
            </div>
        <?php endif; ?>
    </div>
</main>
