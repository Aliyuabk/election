<?php
// ============================================================
// SENATORIAL COORDINATOR - POLLING UNIT STATUS
// ============================================================
...
// ============================================================
// GET POLLING UNIT STATUS WITH FILTERS
// ============================================================
...
if ($status_filter === 'submitted') {
    $status_conditions[] = "r.id IS NOT NULL";
} elseif ($status_filter === 'verified') {
    $status_conditions[] = "r.status = 'verified'";
} elseif ($status_filter === 'pending') {
    $status_conditions[] = "r.id IS NULL OR r.status = 'pending'";
} elseif ($status_filter === 'flagged') {
    $status_conditions[] = "r.status = 'flagged'";
} elseif ($status_filter === 'checked_in') {
    $status_conditions[] = "c.id IS NOT NULL";
} elseif ($status_filter === 'not_checked_in') {
    $status_conditions[] = "c.id IS NULL";
}

if (!empty($lga_filter) && $lga_filter !== 'all') {
    $status_conditions[] = "l.id = ?";
    $params[] = $lga_filter;
}

if (!empty($ward_filter) && $ward_filter !== 'all') {
    $status_conditions[] = "w.id = ?";
    $params[] = $ward_filter;
}

$where_clause = !empty($status_conditions) ? "AND " . implode(" AND ", $status_conditions) : "";
$lga_condition = $lga_list !== '0' ? "AND l.id IN ($lga_list)" : "AND 1=0";
...
try {
    if ($lga_list !== '0') {
        $query = "
            ...
            WHERE l.is_active = 1 AND w.is_active = 1 AND pu.is_active = 1
            $lga_condition
            $where_clause
...
foreach ($polling_units as $pu) {
    if ($pu['result_id']) {
        ...
        if ($pu['result_status'] === 'verified') $status_stats['verified']++;
        elseif ($pu['result_status'] === 'flagged') $status_stats['flagged']++;
    } else {
        $status_stats['pending']++;
    }
    
    if ($pu['checkin_id']) {
        $status_stats['checked_in']++;
    } else {
        $status_stats['not_checked_in']++;
    }
}
    ...
    <div class="main-content-inner">
        ...
        <div class="stats-grid">
            <div class="stat-card blue">
                ...
                <div class="stat-number"><?php echo number_format($status_stats['total']); ?></div>
                <div class="stat-label">Total PUs</div>
            </div>
            <div class="stat-card green">
                <div class="stat-icon"><i class="fas fa-check-circle"></i></div>
                <div class="stat-number"><?php echo number_format($status_stats['verified']); ?></div>
                <div class="stat-label">Verified</div>
            </div>
            <div class="stat-card yellow">
                <div class="stat-icon"><i class="fas fa-clock"></i></div>
                <div class="stat-number"><?php echo number_format($status_stats['pending']); ?></div>
                <div class="stat-label">Pending</div>
            </div>
            <div class="stat-card orange">
                <div class="stat-icon"><i class="fas fa-flag"></i></div>
                <div class="stat-number"><?php echo number_format($status_stats['flagged']); ?></div>
                <div class="stat-label">Flagged</div>
            </div>
            <div class="stat-card teal">
                <div class="stat-icon"><i class="fas fa-sign-in-alt"></i></div>
                <div class="stat-number"><?php echo number_format($status_stats['checked_in']); ?></div>
                <div class="stat-label">Checked In</div>
            </div>
            <div class="stat-card red">
                <div class="stat-icon"><i class="fas fa-clock"></i></div>
                <div class="stat-number"><?php echo number_format($status_stats['not_checked_in']); ?></div>
        ...
        <div class="section-card" style="margin-bottom:20px;">
            ...
            <div class="filters-row">
                <select id="filterLGA" onchange="window.location.href='?status=<?php echo $status_filter; ?>&lga='+this.value+'&ward=<?php echo $ward_filter; ?>'">
                ...
                <select id="filterWard" onchange="window.location.href='?status=<?php echo $status_filter; ?>&lga=<?php echo $lga_filter; ?>&ward='+this.value">
                ...
                <select id="filterStatus" onchange="window.location.href='?status='+this.value+'&lga=<?php echo $lga_filter; ?>&ward=<?php echo $ward_filter; ?>'">
                    <option value="all" <?php echo empty($status_filter) ? 'selected' : ''; ?>>All Status</option>
                    ...
                    <option value="not_checked_in" <?php echo ($status_filter === 'not_checked_in') ? 'selected' : ''; ?>>Not Checked In</option>
                </select>
            </div>
        </div>
        ...
        <div class="section-card">
            <div class="card-header">
                <h3><i class="fas fa-list"></i> Polling Units Status</h3>
                            ...
                            <tr>
                                <td colspan="10">
                                    <div class="empty-state">
                                        ...
                                        <p>No polling units found matching the selected filters.</p>
                                    </div>
                                </td>
                            </tr>
                        <?php endif; ?>
