<?php
// ============================================================
// SENATORIAL COORDINATOR - CREATE BROADCAST
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $send_via = isset($_POST['send_via']) ? (array)$_POST['send_via'] : ['email', 'in_app'];
    ...
    if (empty($send_via)) $errors[] = 'At least one delivery method is required.';
    ...
    if (empty($errors)) {
        try {
            ...
            logActivity($user_id, 'broadcast_created', "Created broadcast: $title (ID: $broadcast_id)");
            
            if ($status === 'scheduled') {
                $success = "Broadcast scheduled successfully! It will be sent at the scheduled time.";
            } else {
                $success = "Broadcast created as draft. You can send it later.";
                // If not scheduled, redirect to send page
                if (isset($_POST['send_now']) && $_POST['send_now'] === '1') {
                    header("Location: broadcasts-send.php?id=$broadcast_id");
                    exit();
                }
            }
...
// ============================================================
// GET RECIPIENT COUNT FOR TARGET
// ============================================================
...
// Get initial recipient count if form data exists
$recipient_count = 0;
if (!empty($form_data['target_audience'])) {
    $recipient_count = getRecipientCount(
        $form_data['target_audience'],
        $form_data['target_ids'] ?? [],
        $form_data['target_role_id'] ?? 0,
        $tenant_id,
        $lga_list
    );
}
    ...
    <div class="main-content-inner">
        ...
        <div class="form-container">
            ...
            <div class="form-subtitle">
                Create a broadcast message to send to coordinators and agents in your senatorial district.
            </div>
            ...
            <form method="POST" action="" id="broadcastForm">
                <div class="form-grid">
                    ...
                    <div class="form-group full-width">
                        ...
                        <input type="text" name="title" placeholder="e.g., Election Day Update" value="<?php echo htmlspecialchars($form_data['title'] ?? ''); ?>" required>
                    </div>
                    
                    <div class="form-group full-width">
                        <label>Message <span class="required">*</span></label>
                        <textarea name="message" placeholder="Type your broadcast message here..." required><?php echo htmlspecialchars($form_data['message'] ?? ''); ?></textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="priority">
                            ...
                            <option value="emergency" <?php echo (isset($form_data['priority']) && $form_data['priority'] == 'emergency') ? 'selected' : ''; ?>>Emergency</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group full-width">
                        <div class="target-options">
                            <label>
                                <input type="radio" name="target_audience" value="all" <?php echo (!isset($form_data['target_audience']) || $form_data['target_audience'] == 'all') ? 'checked' : ''; ?> onchange="updateTargetFields()">
                                All Personnel
                            </label>
                            <label>
                                <input type="radio" name="target_audience" value="lga" <?php echo (isset($form_data['target_audience']) && $form_data['target_audience'] == 'lga') ? 'checked' : ''; ?> onchange="updateTargetFields()">
                                Specific LGAs
                            </label>
                            <label>
                                <input type="radio" name="target_audience" value="role_specific" <?php echo (isset($form_data['target_audience']) && $form_data['target_audience'] == 'role_specific') ? 'checked' : ''; ?> onchange="updateTargetFields()">
                                Specific Role
                            </label>
                        </div>
                    ...
                    <!-- LGA Selection -->
                    <div class="form-group full-width <?php echo (!isset($form_data['target_audience']) || $form_data['target_audience'] != 'lga') ? 'hidden' : ''; ?>" id="lgaTargetGroup">
                        <label>Select LGAs</label>
                        <select name="target_ids[]" id="lgaTargetSelect" multiple style="height:100px;">
                            <?php foreach ($lgas as $lga): ?>
                                <option value="<?php echo $lga['id']; ?>" <?php echo (isset($form_data['target_ids']) && in_array($lga['id'], $form_data['target_ids'])) ? 'selected' : ''; ?>>
                    ...
                    <!-- Role Selection -->
                    <div class="form-group full-width <?php echo (!isset($form_data['target_audience']) || $form_data['target_audience'] != 'role_specific') ? 'hidden' : ''; ?>" id="roleTargetGroup">
                        <label>Select Role</label>
                        <select name="target_role_id" id="roleTargetSelect">
                            <option value="">Select Role...</option>
                            <?php foreach ($roles as $role): ?>
                                <option value="<?php echo $role['id']; ?>" <?php echo (isset($form_data['target_role_id']) && $form_data['target_role_id'] == $role['id']) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group full-width">
                        ...
                        <div style="display:flex;gap:16px;flex-wrap:wrap;padding:8px 0;">
                            <label style="font-weight:400;display:flex;align-items:center;gap:6px;">
                                <input type="checkbox" name="send_via[]" value="email" <?php echo (!isset($form_data['send_via']) || in_array('email', $form_data['send_via'])) ? 'checked' : ''; ?>>
                                Email
                            </label>
                            <label style="font-weight:400;display:flex;align-items:center;gap:6px;">
                                <input type="checkbox" name="send_via[]" value="in_app" <?php echo (!isset($form_data['send_via']) || in_array('in_app', $form_data['send_via'])) ? 'checked' : ''; ?>>
                                In-App Notification
                            </label>
                            <label style="font-weight:400;display:flex;align-items:center;gap:6px;">
                                <input type="checkbox" name="send_via[]" value="sms" <?php echo (isset($form_data['send_via']) && in_array('sms', $form_data['send_via'])) ? 'checked' : ''; ?>>
                                SMS
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="form-group full-width">
                        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                            <label style="display:flex;align-items:center;gap:6px;font-weight:400;">
                                <input type="checkbox" name="schedule" value="1" <?php echo (isset($form_data['schedule']) && $form_data['schedule']) ? 'checked' : ''; ?> onchange="toggleSchedule()">
                                Schedule for later
                            </label>
                            <div id="scheduleDatetime" class="<?php echo (!isset($form_data['schedule']) || !$form_data['schedule']) ? 'hidden' : ''; ?>">
                                <input type="datetime-local" name="scheduled_at" value="<?php echo htmlspecialchars($form_data['scheduled_at'] ?? ''); ?>">
                            </div>
                        </div>
                    </div>
                </div>
