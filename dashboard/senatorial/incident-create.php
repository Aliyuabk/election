<?php
// ============================================================
// SENATORIAL COORDINATOR - CREATE INCIDENT
// ============================================================
...
// ============================================================
// GET WARDS AND PUS FOR DROPDOWN
// ============================================================
$wards = [];
$pus = [];
...
if ($lga_list !== '0') {
    try {
        ...
        $stmt = $db->prepare("
            ...
            WHERE w.lga_id IN ($lga_list) AND w.is_active = 1
        ...
        error_log("Error fetching wards/PUs: " . $e->getMessage());
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($incident_type)) {
        ...
        try {
            // Get state_id from LGA
            $stmt = $db->prepare("SELECT state_id FROM lgas WHERE id = ?");
            $stmt->execute([$lga_id]);
            $lga = $stmt->fetch();
            $state_id_from_lga = $lga ? $lga['state_id'] : $state_id;
    ...
    <div class="main-content-inner">
        ...
        <div class="page-header">
            <div>
                <h2>
                    ...
                    Report Incident
                    <small><?php echo htmlspecialchars($district_name); ?></small>
                </h2>
            </div>
        ...
        <?php if (isset($_GET['success'])): ?>
            <div class="alert alert-success">
                ...
                <div>Incident reported successfully!</div>
            </div>
        <?php endif; ?>
        ...
        <div class="form-container">
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title" placeholder="Brief title of the incident" value="<?php echo htmlspecialchars($_POST['title'] ?? ''); ?>" required>
                </div>

                <div class="form-group">
                    <label>Description <span class="required">*</span></label>
                    <textarea name="description" placeholder="Detailed description of what happened" required><?php echo htmlspecialchars($_POST['description'] ?? ''); ?></textarea>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="lga_id" id="lga_id" required>
                            ...
                            <?php foreach ($lga_ids as $id => $name): ?>
                                <option value="<?php echo $id; ?>" <?php echo ($_POST['lga_id'] ?? 0) == $id ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select name="ward_id" id="ward_id">
                            ...
                            <?php foreach ($wards as $ward): ?>
                                <option value="<?php echo $ward['id']; ?>" data-lga="<?php echo $ward['lga_id']; ?>" <?php echo ($_POST['ward_id'] ?? 0) == $ward['id'] ? 'selected' : ''; ?>>
                ...
                <div class="form-group">
                    ...
                    <select name="pu_id" id="pu_id">
                        ...
                        <?php foreach ($pus as $pu): ?>
                            <option value="<?php echo $pu['id']; ?>" data-ward="<?php echo $pu['ward_id']; ?>" <?php echo ($_POST['pu_id'] ?? 0) == $pu['id'] ? 'selected' : ''; ?>>
                ...
                <div class="form-group">
                    <div class="checkbox-group">
                        <input type="checkbox" name="is_panic" id="is_panic" value="1" <?php echo isset($_POST['is_panic']) ? 'checked' : ''; ?>>
                        <label for="is_panic">
...
document.getElementById('lga_id').addEventListener('change', function() {
    ...
    for (var i = 0; i < puSelect.options.length; i++) {
        ...
        // Temporarily show all, will be filtered by ward
