<?php
// ============================================================
// SENATORIAL COORDINATOR - RESULTS BY LGA
// ============================================================
...
if ($selected_lga > 0) {
    try {
        ...
        // Get results by Ward within this LGA
        $stmt = $db->prepare("
            SELECT 
                w.id as ward_id,
                w.name as ward_name,
...
foreach ($ward_results as $ward) {
    ...
    $summary['submitted'] += $ward['results_submitted'];
    $summary['verified'] += $ward['results_verified'];
    $summary['total_votes'] += $ward['total_votes'];
    $summary['valid_votes'] += $ward['valid_votes'];
    $summary['rejected_votes'] += $ward['rejected_votes'];
}
    ...
    <div class="main-content-inner">
        ...
        <?php if ($selected_lga > 0 && $lga_data): ?>
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;">
                    <div>
                        <h3 style="font-size:1.1rem;font-weight:700;margin:0;">
                            <?php echo htmlspecialchars($lga_data['name']); ?>
                        </h3>
                        <p style="color:var(--gray-500);font-size:0.85rem;margin:4px 0 0;">
                            <i class="fas fa-hashtag"></i> <?php echo htmlspecialchars($lga_data['code']); ?>
                            <span style="margin-left:16px;">
                                <i class="fas fa-layer-group"></i> <?php echo number_format($lga_data['ward_count'] ?? 0); ?> Wards
                            </span>
                            <span style="margin-left:16px;">
                                <i class="fas fa-flag-checkered"></i> <?php echo number_format($lga_data['pu_count'] ?? 0); ?> PUs
                            </span>
                        </p>
                    </div>
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card teal">
                    <div class="stat-number"><?php echo number_format($summary['valid_votes']); ?></div>
                    <div class="stat-label">Valid Votes</div>
                </div>
                <div class="stat-card yellow">
                    <div class="stat-number"><?php echo number_format($summary['rejected_votes']); ?></div>
            ...
            <div class="section-card">
                <div class="card-header">
                    <h3><i class="fas fa-layer-group" style="color:var(--primary);margin-right:6px;"></i> Results by Ward</h3>
                    <span style="font-size:0.75rem;color:var(--gray-400);"><?php echo count($ward_results); ?> wards</span>
                </div>
                <div class="table-wrap">
                    <table class="table">
                        ...
                        <tbody>
                            <?php if (count($ward_results) > 0): ?>
                                <?php $i = 1; foreach ($ward_results as $ward): 
                                    $rate = $ward['total_pus'] > 0 ? round(($ward['results_submitted'] / $ward['total_pus']) * 100, 1) : 0;
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($ward['ward_code']); ?></td>
                                        <td><?php echo number_format($ward['total_pus']); ?></td>
                                        <td><?php echo number_format($ward['results_submitted']); ?></td>
                                        <td><?php echo number_format($ward['results_verified']); ?></td>
                                        <td><?php echo number_format($ward['total_votes'] ?? 0); ?></td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="results-by-ward.php?ward_id=<?php echo $ward['ward_id']; ?>" class="btn-sm">View</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="10">
                                        <div class="empty-state">
                                            ...
                                            <p>No ward results found for this LGA.</p>
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
        ...
        <?php else: ?>
            <div class="empty-state">
                <i class="fas fa-map-marker-alt"></i>
                <h3>Select an LGA</h3>
                <p>Choose an LGA from the dropdown above to view results.</p>
            </div>
        <?php endif; ?>
    </div>
</main>
