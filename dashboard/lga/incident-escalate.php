<?php
// ============================================================
// LGA COORDINATOR - ESCALATE INCIDENT
// ============================================================
...
// Only allow escalation if not already escalated or closed
if (in_array($incident['status'], ['escalated', 'closed', 'resolved'])) {
...
// Get state coordinators for escalation
$state_coordinators = [];
try {
    $stmt = $db->prepare("
        ...
        WHERE r.level = 'state' AND u.state_id = (SELECT state_id FROM lgas WHERE id = ?)
        AND u.status = 'active' AND u.deleted_at IS NULL
        ORDER BY u.first_name ASC
    ");
    $stmt->execute([$lga_id]);
    $state_coordinators = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    error_log("Error fetching state coordinators: " . $e->getMessage());
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif ($escalate_to <= 0) {
        $error = 'Please select who to escalate to.';
    } else {
        try {
            $stmt = $db->prepare("
                UPDATE incidents 
            ...
            ");
            $stmt->execute([$escalate_to, $incident_id, $tenant_id]);
            
            // Add notes
            $notes = $incident['resolution_notes'] ?? '';
            $new_notes = $notes . "\n[" . date('Y-m-d H:i:s') . "] ESCALATED to State Coordinator: " . $escalation_reason;
            ...
            logActivity($user_id, 'incident_escalated', 
                "Escalated incident #$incident_id to State Coordinator - Reason: $escalation_reason",
        ...
        <div class="escalate-container">
            ...
            <div class="escalate-card">
                ...
                <div class="warning-box">
                    <i class="fas fa-exclamation-triangle"></i>
                    <strong>Escalation Warning:</strong> Escalating this incident will move it to the State Coordinator for review and action.
                </div>
                ...
                <form method="POST" action="">
                    <div class="form-group">
                        ...
                        <select name="escalate_to" required>
                            ...
                            <?php foreach ($state_coordinators as $coordinator): ?>
                            ...
                            <?php endforeach; ?>
                            <?php if (empty($state_coordinators)): ?>
                                <option value="0">No State Coordinators available</option>
                            <?php endif; ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="escalation_notes" placeholder="Any additional information for the State Coordinator..."></textarea>
                    </div>
