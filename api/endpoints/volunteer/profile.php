    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        if ($profile = $result->fetch_assoc()) {
            ...
            $coordStmt = $conn->prepare("
                SELECT CONCAT(first_name, ' ', last_name) as coordinator_name
                FROM users u
                JOIN volunteer_assignments va ON u.id = va.coordinator_id
            ...
            if ($coord = $coordResult->fetch_assoc()) {
                $profile['coordinator_name'] = $coord['coordinator_name'];
            } else {
